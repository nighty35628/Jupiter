...
// Devtools must be lazily required in development only. Eagerly importing
// `react-devtools-core` runs its top-level WebSocket bootstrap on module load
// and races with host apps that have their own devtools wiring; dynamic import
// inside the NODE_ENV guard lets dead-code elimination strip it from
...
if (process.env.NODE_ENV === 'development') {
  try {
    ...
    if (error instanceof Error && (error as NodeJS.ErrnoException).code === 'ERR_MODULE_NOT_FOUND') {
      ...
      console.warn(
        `
NODE_ENV=development, but \`react-devtools-core\` is not installed —
the optional DevTools bridge cannot connect. Install it as a dev
dependency to enable component inspection:
...
const diff = (before: AnyObject, after: AnyObject): AnyObject | undefined => {
  ...
  let changed: AnyObject | undefined

  for (const key in before) {
    if (!Object.hasOwn(before, key)) continue
    if (!after || !Object.hasOwn(after, key)) {
...
export function isDebugRepaintsEnabled(): boolean {
  if (debugRepaints === undefined) {
    debugRepaints = isEnvTruthy(process.env.JUPITER_DEBUG_REPAINTS)
  }
...
const COMMIT_LOG = process.env.JUPITER_COMMIT_LOG
let _commits = 0
let _lastLog = 0
let _lastCommitAt = 0
let _maxGapMs = 0
...
>({
  ...
  removeChild(node: DOMElement, removeNode: DOMElement | TextNode) {
    removeChildNode(node, removeNode)
    cleanupYogaNode(removeNode as DOMElement)
    if (removeNode.nodeName !== '#text') {
      const root = getRootNode(node)
      root.focusManager!.handleNodeRemoved(removeNode as DOMElement, root)
    }
  },
