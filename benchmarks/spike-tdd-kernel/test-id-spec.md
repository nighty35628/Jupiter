...
### C. user annotation — `// @jupiter-test-id: foo`
...
If the test source contains `// @jupiter-test-id: <slug>` directly above the `it(`/`test(`, that slug overrides the default.

```ts
// @jupiter-test-id: bang.parses-leading-bang
...
- **Greenfield**: model uses the default unless the user requests stability. `jupiter doctor` could surface a warning when a `test_id` would be lost.
...
- Cross-runner support (jest, mocha). Jupiter workspaces today are predominantly vitest; ship vitest-only first.
