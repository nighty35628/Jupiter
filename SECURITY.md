...
If you find a security issue in Jupiter, please report it privately rather than opening a public issue or discussion thread.
...
- the version (`jupiter --version`) and platform you observed it on
...
Only the latest published minor of `jupiter` on npm is actively maintained. If you're on something older, please reproduce on the latest before reporting.

## Scope

**In scope:**

- The published `jupiter` npm package and its CLI / TUI
...
- Denial-of-service via deliberately oversized prompts or tool inputs (Jupiter is a single-user CLI; there's no multi-tenant boundary to defend)

## Hardening notes

A few practical reminders for users running Jupiter:

- API keys live in `~/.jupiter/config.json`. Treat that file like any other credential store.
...
- Hooks (`PreToolUse`, etc.) execute arbitrary shell scripts the user has configured. Audit `.jupiter/settings.json` before running Jupiter in a directory you didn't author.
