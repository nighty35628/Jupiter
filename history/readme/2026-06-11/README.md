...
- 文件系统工具现在会在沙盒判定前展开 `~/...` 和全角 `～/...`，避免用户目录路径被误当成工作区内的普通相对路径。
- 编辑工具的路径审查同步支持用户目录展开，`edit_file`、`write_file` 和 `multi_edit` 的越界确认更贴近真实目标文件。
- 桌面空状态的 hero 输入框和 placeholder 居中显示，让初始工作台布局更稳定。
- 设置页归档列表新增一键清空入口，只清理已归档会话；会话导入默认排除 Claude subagent 记录，并支持选择具体候选会话导入。
- 桌面端、根包、Tauri 配置、CHANGELOG 和 Release notes 版本统一升级到 `0.99.3`。
...
- Filesystem tools now expand `~/...` and full-width `～/...` before sandbox checks, avoiding accidental treatment
  of home-directory paths as normal workspace-relative paths.
- Edit-tool path review uses the same home-path expansion, so `edit_file`, `write_file`, and `multi_edit`
  confirmations point at the real target file.
- The desktop empty-state hero composer and placeholder are centered for a steadier initial workbench layout.
- The settings archive list now has a clear-archive action, while session import discovery excludes Claude
  subagent records by default and supports importing selected candidates.
- Desktop, root package, Tauri config, CHANGELOG, and Release notes versions are aligned on `0.99.3`.
