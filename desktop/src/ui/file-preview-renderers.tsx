...
function DocxPreview({ url }: { url: string }): React.ReactElement {
  ...
  useEffect(() => {
    ...
    void Promise.all([import("docx-preview"), fetch(url)])
      .then(async ([docx, response]) => {
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const buffer = await response.arrayBuffer();
        if (cancelled) return;
        await docx.renderAsync(buffer, target, undefined, {
          inWrapper: true,
          ignoreWidth: true,
          ignoreHeight: true,
