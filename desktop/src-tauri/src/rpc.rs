...
fn resolve_cli(app: &AppHandle) -> Result<(String, Vec<String>)> {
    if let Ok(custom) = env::var("JUPITER_CLI") {
        ...
        let program = parts
            .next()
            .ok_or_else(|| anyhow!("JUPITER_CLI is empty"))?;
    ...
    // `open target/debug/Jupiter.app` can start with a cwd outside the repo,
    // so also walk upward from the executable path.
    ...
    if let Ok(exe) = env::current_exe() {
        for ancestor in exe.ancestors() {
            candidates.push(ancestor.join("dist/cli/index.js"));
        }
    }
    ...
    eprintln!("[jupiter] resolved node: {}", node_path.display());
