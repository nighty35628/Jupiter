  ...
  switch (action.t) {
    ...
    case "rpc_exit":
      return {
        ...
        messages: [
          ...state.messages,
          { kind: "error", message: `jupiter exited (code ${action.code ?? "?"})` },
        ],
      };
    case "incoming":
...
export function App() {
  ...
  const [sideCollapsed, setSideCollapsed] = useState(
    () => localStorage.getItem("jupiter.sideCollapsed") === "1",
  );
  const [ctxCollapsed, setCtxCollapsed] = useState(
    () => localStorage.getItem("jupiter.ctxCollapsed") === "1",
  );
  ...
  useEffect(() => {
    localStorage.setItem("jupiter.sideCollapsed", sideCollapsed ? "1" : "0");
  }, [sideCollapsed]);

  useEffect(() => {
    localStorage.setItem("jupiter.ctxCollapsed", ctxCollapsed ? "1" : "0");
  }, [ctxCollapsed]);
