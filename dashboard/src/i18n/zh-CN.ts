export const zhCN = {
  ...
  overview: {
    ...
    standaloneDesc:
      "只读磁盘视图。在 jupiter code 内启动 /dashboard 以获取实时会话状态、MCP 和工具。",
    cockpit: "驾驶舱",
    balance: "余额",
    tokens7d: "tokens · 7 天",
    cacheHit: "缓存命中",
    ...
    budget: "预算",
    currentSession: "当前会话",
    noSession: "无活跃会话 — 在 jupiter code 内执行 /dashboard 进行连接。",
    promptTok: "提示 tokens",
  ...
  usage: {
    ...
    noData: "暂无用量数据 — 在 jupiter chat / code / run 中执行一轮，然后刷新。",
    windows: "滚动窗口",
    colWindow: "时间范围",
    colTurns: "轮次",
    colCacheHit: "缓存命中",
  ...
  sessions: {
    ...
    attachRequired:
      "实时会话操作需要已连接的 CLI 会话。请通过 jupiter chat 启动，或在 TUI 会话中打开仪表盘。",
    activeChip: "当前",
    activePill: "当前",
  },
  tools: {
  ...
  permissions: {
    ...
    failed: "权限失败：{error}",
    yoloTitle: "完全控制权限",
    yoloDesc: "所有 shell 命令自动运行，不再询问。可切回“审阅”重新启用确认。",
    project: "项目",
    builtin: "内置",
    addPrefix: "添加前缀",
    ...
    standaloneWarning:
      "修改操作需要在活跃的 jupiter code 会话内执行 /dashboard — 独立模式的 jupiter dashboard 无法确定要编辑哪个项目的允许列表。",
  },
  mcp: {
    loading: "加载 MCP…",
    ...
    saved: "已保存",
    savedRestart: "已保存 — 重启 jupiter code 以桥接此服务器",
    ...
    whyUnbridged: "为什么未桥接？",
    whyUnbridgedDesc:
      "此规格存在于您的 config.json 中，但未桥接到实时会话。MCP 服务器在 jupiter code 启动时连接；仪表盘本身无法生成子进程。",
    whyUnbridgedHint: "激活方法：重启 jupiter code，然后刷新此仪表盘。",
    bridgeFailed: "桥接失败",
    bridgeFailedTitle: "桥接失败 · 在配置中",
    bridgeFailedHint:
      "已尝试桥接但失败。常见原因：URL 错误、需要鉴权、上游 404 / 5xx、本地命令缺失。修复后重启 jupiter code 重试。",
    ...
    marketplaceEnvHint: "下次启动 `jupiter code` 之前在 shell 里设好，桥接的服务器才能正常鉴权。",
    marketplaceRestartHint:
      "已写入 ~/.jupiter/config.json。重启 `jupiter code` 后服务器才会真正桥接（热重载在路线图上）。",
  },
  memory: {
    loading: "加载记忆…",
    files: "记忆 · {count} 个文件",
    ...
    noFiles: "暂无记忆文件。",
    pickHint: "选择左侧的记忆文件。",
    pickDesc: "项目 JUPITER.md 可提交；全局笔记存储在 ~/.jupiter/memory/。",
    chars: "{count} 个字符",
    saved: "已保存 {scope}",
  ...
  hooks: {
    ...
    colScript: "脚本",
    noProject: "无活跃项目 — 在 jupiter code 中打开 /dashboard 以编辑项目钩子。",
    saveReload: "保存并重载",
    discard: "放弃更改",
  ...
  skills: {
    ...
    builtinDesc:
      "内置技能随 Jupiter 一起发布；模型会自动识别。如需自定义，请创建同名的项目或全局技能。",
  ...
  semantic: {
    ...
    installOllamaDesc: "Jupiter 不会为您运行包管理器。请先安装 Ollama，然后返回：",
    ...
    remoteProviderDesc:
      "在这里配置 OpenAI-Compatible embeddings 的完整 URL。Jupiter 会严格使用你提供的 URL 发起请求。",
    ollama: "Ollama",
    binary: "二进制",
    found: "已找到",
    missing: "缺失",
  ...
  editMode: {
    ...
    yolo: "完全控制",
    yoloHint: "完全控制权限 · 编辑和 shell 都自动运行，不再询问",
  },
};
