...
describe("/permissions slash handler", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "jupiter-perms-slash-"));
    cfgPath = join(dir, "config.json");
    projectRoot = join(dir, "project");
    // Redirect ~/.jupiter → temp dir so the handler's calls (which use
  ...
  it("bare /permissions lists project entries with 1-based indices", () => {
    addProjectShellAllowed(projectRoot, "npm run build", join(dir, ".jupiter", "config.json"));
    addProjectShellAllowed(projectRoot, "deploy.sh", join(dir, ".jupiter", "config.json"));
    const result = handleSlash("permissions", [], makeLoop(), {
      codeRoot: projectRoot,
      editMode: "review",
    });
  ...
  it("/permissions add persists a new prefix", () => {
    ...
    expect(loadProjectShellAllowed(projectRoot, join(dir, ".jupiter", "config.json"))).toContain(
      "npm run build",
    );
  });
  ...
  it("/permissions add notes when the target is already in the builtin list", () => {
    ...
    expect(loadProjectShellAllowed(projectRoot, join(dir, ".jupiter", "config.json"))).toEqual([]);
  });

  it("/permissions remove drops by exact prefix", () => {
    const cfgFile = join(dir, ".jupiter", "config.json");
  ...
  it("mutating subcommands refuse without a codeRoot", () => {
    ...
    expect(r1.info).toMatch(/only available inside `jupiter code`/);
    const r2 = handleSlash("permissions", ["remove", "lint"], makeLoop(), {});
    expect(r2.info).toMatch(/only available inside `jupiter code`/);
    const r3 = handleSlash("permissions", ["clear", "confirm"], makeLoop(), {});
    expect(r3.info).toMatch(/only available inside `jupiter code`/);
  });
