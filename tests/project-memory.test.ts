/** JUPITER.md project-memory loader — filesystem-backed tests in a temp dir. */
...
describe("project-memory", () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "jupiter-mem-"));
  ...
  describe("readProjectMemory", () => {
    it("returns null when JUPITER.md is absent", () => {
      expect(readProjectMemory(root)).toBeNull();
    });

    it("returns null when JUPITER.md is empty / whitespace-only", () => {
    ...
    it("falls back to .claude/CLAUDE.md when JUPITER.md is absent", () => {
    ...
    it("falls back to AGENTS.md when no JUPITER.md or CLAUDE.md exists", () => {
    ...
    it("prefers JUPITER.md over CLAUDE.md candidates", () => {
      writeFileSync(join(root, "JUPITER.md"), "jupiter wins\n", "utf8");
      mkdirSync(join(root, ".claude"));
      ...
      const mem = readProjectMemory(root);
      expect(mem?.content).toBe("jupiter wins");
      expect(mem?.path.endsWith("JUPITER.md")).toBe(true);
    });
  ...
  describe("resolveProjectMemoryWritePath", () => {
    it("returns JUPITER.md path when no candidate exists yet (fresh project)", () => {
      expect(resolveProjectMemoryWritePath(root).endsWith("JUPITER.md")).toBe(true);
    });
    ...
    it("JUPITER.md still wins as the write target when it coexists with AGENTS.md", () => {
      writeFileSync(join(root, "JUPITER.md"), "x", "utf8");
  ...
  describe("memoryEnabled", () => {
    ...
    it.each(["off", "false", "0"])("returns false for JUPITER_MEMORY=%s", (val) => {
      process.env.JUPITER_MEMORY = val;
  ...
  describe("applyProjectMemory", () => {
    ...
    it("appends a '# Project memory' fenced block when the file exists", () => {
      ...
      expect(out).toMatch(/# Project memory \(JUPITER\.md\)/);
    ...
    it("header reflects AGENTS.md when that's the file we fell back to", () => {
      ...
      expect(out).not.toMatch(/# Project memory \(JUPITER\.md\)/);
      expect(out).toContain("open-spec rules");
    });

    it("no-ops when JUPITER_MEMORY=off, even with a file present", () => {
