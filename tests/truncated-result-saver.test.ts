...
describe("saveTruncatedResult", () => {
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), "jupiter-trunc-save-"));
  });
  ...
  it("writes content to .jupiter/truncated-results/", () => {
    ...
    expect(path).toMatch(/^\.jupiter\/truncated-results\/\d+-[a-f0-9]+-web_search\.txt$/);
  ...
  it("creates the directory if missing", () => {
    ...
    expect(existsSync(join(rootDir, ".jupiter", "truncated-results"))).toBe(true);
  ...
  it("falls back to ~/.jupiter when rootDir is the filesystem root", () => {
    const origHome = process.env.HOME;
    const origUserProfile = process.env.USERPROFILE;
    const fakeHome = mkdtempSync(join(tmpdir(), "jupiter-trunc-home-"));
    ...
    try {
      ...
      expect(path).toMatch(/\.jupiter\/truncated-results\//);
      expect(path.startsWith("/.jupiter")).toBe(false);
  ...
  it("falls back to ~/.jupiter when rootDir is empty", () => {
...
describe("cleanupOldResults", () => {
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), "jupiter-trunc-cln-"));
  ...
  it("removes files older than maxAgeMs", () => {
    const dir = join(rootDir, ".jupiter", "truncated-results");
  ...
  it("is a no-op on missing directory", () => {
    const missing = mkdtempSync(join(tmpdir(), "jupiter-trunc-missing-"));
