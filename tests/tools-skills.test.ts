...
function writeSkill(baseDir: string, name: string, description: string, body: string): void {
  const dir = join(baseDir, ".jupiter", "skills", name);
...
describe("run_skill tool", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "jupiter-skilltool-"));
    projectRoot = mkdtempSync(join(tmpdir(), "jupiter-skilltool-proj-"));
  });
  ...
  it("returns a custom path skill when customSkillPaths is passed", async () => {
    const custom = mkdtempSync(join(tmpdir(), "jupiter-skilltool-custom-"));
    try {
...
describe("install_skill tool", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "jupiter-installskill-"));
    projectRoot = mkdtempSync(join(tmpdir(), "jupiter-installskill-proj-"));
  });
...
describe("built-in subagent tools (explore / research / review / security_review)", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "jupiter-builtin-subagent-"));
  });
