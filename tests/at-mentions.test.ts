...
describe("expandAtMentions", () => {
  let root: string;
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "jupiter-at-mentions-"));
...
describe("listFilesSync", () => {
  let root: string;
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "jupiter-listfiles-"));
    mkdirSync(join(root, "src"), { recursive: true });
...
describe("listFilesWithStatsAsync", () => {
  let root: string;
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "jupiter-listfiles-async-"));
    mkdirSync(join(root, "src"), { recursive: true });
    mkdirSync(join(root, "src", "cli"), { recursive: true });
    mkdirSync(join(root, "node_modules", "foo"), { recursive: true });
    writeFileSync(join(root, "package.json"), "{}");
...
describe("listDirectory", () => {
  let root: string;
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "jupiter-listdir-"));
...
describe("walkFilesStream", () => {
  let root: string;
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "jupiter-stream-"));
