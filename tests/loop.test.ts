...
describe("CacheFirstLoop (streaming) — tool_call_delta emission", () => {
  ...
  describe("parallel tool dispatch", () => {
    ...
    it("JUPITER_TOOL_DISPATCH=serial forces serial dispatch", async () => {
      const prev = process.env.JUPITER_TOOL_DISPATCH;
      process.env.JUPITER_TOOL_DISPATCH = "serial";
      try {
        const client = makeClient([
          makeMultiToolResponse([
            { name: "slow_read", args: '{"k":1}' },
        ...
        if (prev === undefined) {
          ...
          delete process.env.JUPITER_TOOL_DISPATCH;
        } else process.env.JUPITER_TOOL_DISPATCH = prev;
      }
    });

    it("JUPITER_PARALLEL_MAX caps the chunk size", async () => {
      const prev = process.env.JUPITER_PARALLEL_MAX;
      process.env.JUPITER_PARALLEL_MAX = "2";
      try {
        const client = makeClient([
          makeMultiToolResponse([
            { name: "slow_read", args: '{"k":1}' },
        ...
        if (prev === undefined) {
          // biome-ignore lint/performance/noDelete: env restore must remove the key, not stringify "undefined"
          delete process.env.JUPITER_PARALLEL_MAX;
        } else process.env.JUPITER_PARALLEL_MAX = prev;
      }
    });
  });
});
...
describe("CacheFirstLoop — mid-turn steer injection", () => {
  ...
  it("continues past the former 50-round cap until the model finishes", async () => {
    const toolRounds = 55;
    const responses: FakeResponseShape[] = Array.from({ length: toolRounds }, (_, i) => ({
      content: "",
      tool_calls: [
        {
          id: `call_${i}`,
    ...
    const events: any[] = [];
    for await (const ev of loop.step("finish a tool-heavy task")) {
      events.push(ev);
    }

    expect(events.some((e) => e.role === "warning" && /iteration cap/i.test(e.content ?? ""))).toBe(
      false,
    );
    expect(events.find((e) => e.role === "done")?.content).toBe("Finished after a long tool run.");
    expect(noop).toHaveBeenCalledTimes(toolRounds);
    expect(fetchMock).toHaveBeenCalledTimes(toolRounds + 1);
  });
});
