...
describe("startup-profile", () => {
  const original = process.env.JUPITER_PROFILE_STARTUP;
  ...
  afterEach(() => {
    if (original === undefined) Reflect.deleteProperty(process.env, "JUPITER_PROFILE_STARTUP");
    else process.env.JUPITER_PROFILE_STARTUP = original;
    _resetForTests();
  });

  it("is disabled by default — markPhase + dumpStartupProfile are no-ops", () => {
    Reflect.deleteProperty(process.env, "JUPITER_PROFILE_STARTUP");
