...
function git(cwd: string, args: string[]): string {
  return execFileSync("git", args, {
    ...
    encoding: "utf8",
    env: cleanGitEnv({
      GIT_AUTHOR_NAME: "Jupiter Test",
      GIT_AUTHOR_EMAIL: "jupiter@example.test",
      GIT_COMMITTER_NAME: "Jupiter Test",
      GIT_COMMITTER_EMAIL: "jupiter@example.test",
    }),
  }).trim();
}
...
describe("auto-git-rollback memory guard", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "jupiter-auto-git-root-"));
    home = await mkdtemp(join(tmpdir(), "jupiter-auto-git-home-"));
    tools = new ToolRegistry();
    registerFilesystemTools(tools, { rootDir: root, autoGitRollback: { homeDir: home } });
    readTracker = new ReadTracker();

    git(root, ["init", "-q"]);
    git(root, ["config", "user.name", "Jupiter Test"]);
    git(root, ["config", "user.email", "jupiter@example.test"]);
  ...
  it("ignores ambient Git hook environment variables when checkpointing", async () => {
    const outerRoot = await mkdtemp(join(tmpdir(), "jupiter-auto-git-outer-"));
