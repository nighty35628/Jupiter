...
describe("installProxyIfConfigured", () => {
  ...
  it("layers JUPITER_NO_PROXY on top of system NO_PROXY (app-specific override)", () => {
    const result = installProxyIfConfigured({
      HTTPS_PROXY: "http://example:8080",
      NO_PROXY: "system.corp.example",
      JUPITER_NO_PROXY: "app.specific.example, .jupiter.lan",
    });
    const raws = result?.noProxy.map((p) => p.raw) ?? [];
    expect(raws).toContain("system.corp.example");
    expect(raws).toContain("app.specific.example");
    expect(raws).toContain(".jupiter.lan");
  });
  ...
  it("env JUPITER_PROXY_DEEPSEEK_DIRECT=0 drops deepseek even without config override (#1497)", () => {
    const result = installProxyIfConfigured({
      HTTPS_PROXY: "http://example:8080",
      JUPITER_PROXY_DEEPSEEK_DIRECT: "0",
    });
    const raws = result?.noProxy.map((p) => p.raw) ?? [];
    expect(raws).not.toContain("api.deepseek.com");
...
describe("resolveNoProxy", () => {
  it("returns just defaults when no env vars or extra patterns are set", () => {
    ...
    expect(r.envJupiter).toHaveLength(0);
    expect(r.extra).toHaveLength(0);
    expect(r.all.length).toBe(DEFAULT_NO_PROXY.length);
  });

  it("partitions patterns by source (defaults / env / JUPITER / extra)", () => {
    const r = resolveNoProxy(
      { NO_PROXY: "system.example", JUPITER_NO_PROXY: "app.example" },
    ...
    expect(r.envJupiter.map((p) => p.raw)).toEqual(["app.example"]);
  ...
  it("env JUPITER_PROXY_DEEPSEEK_DIRECT=0 drops the DeepSeek bypass (#1497)", () => {
    const r = resolveNoProxy({ JUPITER_PROXY_DEEPSEEK_DIRECT: "0" }, {});
    expect(matchesNoProxy("api.deepseek.com", r.all)).toBe(false);
    expect(matchesNoProxy("127.0.0.1", r.all)).toBe(true);
  });

  it("env JUPITER_PROXY_DEEPSEEK_DIRECT wins over config when set", () => {
    const r = resolveNoProxy(
      { JUPITER_PROXY_DEEPSEEK_DIRECT: "1" },
      { bypassDeepSeekDirect: false },
    );
    expect(matchesNoProxy("api.deepseek.com", r.all)).toBe(true);
  });
...
describe("resolveBypassDeepSeekDirect (#1497)", () => {
  ...
  it("env false-y values flip the default off", () => {
    for (const v of ["0", "false", "no", "off", "FALSE", "Off"]) {
      expect(resolveBypassDeepSeekDirect({ JUPITER_PROXY_DEEPSEEK_DIRECT: v }, undefined)).toBe(
        false,
      );
    }
  });
  ...
  it("env truthy values force the bypass back on (override config false)", () => {
    for (const v of ["1", "true", "yes", "on", "TRUE", "Yes"]) {
      expect(resolveBypassDeepSeekDirect({ JUPITER_PROXY_DEEPSEEK_DIRECT: v }, false)).toBe(true);
    }
  });

  it("unrecognized env values fall through to config", () => {
    expect(resolveBypassDeepSeekDirect({ JUPITER_PROXY_DEEPSEEK_DIRECT: "maybe" }, false)).toBe(
      false,
    );
    expect(resolveBypassDeepSeekDirect({ JUPITER_PROXY_DEEPSEEK_DIRECT: "maybe" }, undefined)).toBe(
      true,
    );
  });
});
