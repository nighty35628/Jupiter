...
describe("session persistence", () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), "jupiter-session-"));
  ...
  it("sessionPath lives under <home>/.jupiter/sessions", () => {
    const p = sessionPath("demo");
    expect(p).toContain(".jupiter");
  ...
  describe("findSessionsByPrefix", () => {
    ...
    it("returns session names matching the prefix, sorted alpha-reverse", () => {
      ...
      appendSessionMessage("code-jupiter-old", { role: "user", content: "x" });
      appendSessionMessage("code-jupiter-20260430T143200", { role: "user", content: "y" });
      appendSessionMessage("code-jupiter-20260430T154500", { role: "user", content: "z" });

      const result = findSessionsByPrefix("code-jupiter-");
      expect(result).toEqual([
        "code-jupiter-old",
        "code-jupiter-20260430T154500",
        "code-jupiter-20260430T143200",
      ]);
    });
  ...
  describe("desktop resume mtime stability", () => {
    it("does not rewrite a thinking-mode transcript when load-time stamp+prune is a no-op", () => {
      appendSessionMessage("stable-resume", { role: "user", content: "你好" });
      appendSessionMessage("stable-resume", { role: "assistant", content: "你好！" });
      appendSessionMessage("stable-resume", { role: "user", content: "你好" });
      appendSessionMessage("stable-resume", {
        role: "assistant",
        content: "你好！有什么可以帮你的吗？",
        reasoning_content: "respond warmly",
      });
      const path = sessionPath("stable-resume");
      const old = new Date("2026-05-30T00:00:00Z");
