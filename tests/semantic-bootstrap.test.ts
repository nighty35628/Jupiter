...
describe("bootstrapSemanticSearchInCodeMode", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "jupiter-bootstrap-"));
  ...
  it("registers the tool when an index already exists", async () => {
    const semanticDir = join(root, ".jupiter", "semantic");
  ...
  it("silently skips (no prompt) when no index is built — even with Ollama present", async () => {
    ...
    // `jupiter index` command + `/semantic` slash. This is the
    // load-bearing UX guarantee — `npx jupiter code` must be silent
