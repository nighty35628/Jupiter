...
async function call(
  ...
  if (method !== "GET") headers["X-Jupiter-Token"] = TOKEN;
...
describe("dashboard /sessions: new / switch / delete (attached)", () => {
  ...
  beforeEach(async () => {
    dir = mkdtempSync(join(tmpdir(), "jupiter-sess-ops-"));
    // Re-home so listSessions reads from our temp dir, not the user's real ~/.jupiter.
  ...
  it("GET /api/sessions filters out other-workspace sessions when getCurrentCwd is wired", async () => {
    await handle?.close();
    const otherWorkspace = mkdtempSync(join(tmpdir(), "jupiter-other-ws-"));
    try {
      ...
      appendSessionMessage("subagent-sub-tagged-202605170236", { role: "user", content: "x" });
      patchSessionMeta("subagent-sub-tagged-202605170236", { workspace: dir });
      ...
      expect(r.status).toBe(200);
      const names = r.body.sessions.map((s: any) => s.name).sort();
      // gamma (other workspace) + subagent internals MUST be filtered out.
      expect(names).toEqual(["alpha", "beta"]);
    } finally {
