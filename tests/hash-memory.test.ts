...
describe("appendProjectMemory", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "jupiter-hashmem-"));
  });
  ...
  it("creates JUPITER.md with a header and the first bullet when absent", () => {
    const path = join(dir, "JUPITER.md");
    expect(existsSync(path)).toBe(false);
    const result = appendProjectMemory(dir, "always use pnpm");
    expect(result.created).toBe(true);
    expect(result.path).toBe(path);
    const content = readFileSync(path, "utf8");
    expect(content).toContain("# Jupiter project memory");
    expect(content).toMatch(/- always use pnpm\n$/);
  });

  it("appends to an existing JUPITER.md without disturbing earlier content", () => {
    const path = join(dir, "JUPITER.md");
  ...
  it("appends multiple bullets in order across calls", () => {
    ...
    const content = readFileSync(join(dir, "JUPITER.md"), "utf8");
  ...
  it("respects nested rootDir paths (creates JUPITER.md in the given dir, not cwd)", () => {
    const nested = join(dir, "subproject");
    mkdirSync(nested);
    const result = appendProjectMemory(nested, "scoped note");
    expect(result.path).toBe(join(nested, "JUPITER.md"));
    expect(existsSync(join(dir, "JUPITER.md"))).toBe(false);
  });
});
...
describe("appendGlobalMemory", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "jupiter-globalmem-"));
  });
  ...
  it("creates ~/.jupiter/JUPITER.md (with parent dir) when missing", () => {
    ...
    expect(result.path).toBe(path);
    const content = readFileSync(path, "utf8");
    expect(content).toContain("# Jupiter global memory");
    expect(content).toMatch(/- always use pnpm\n$/);
  });
  ...
  it("uses os.homedir() when no override is passed (smoke check)", () => {
    ...
    expect(path).toMatch(/[/\\]\.jupiter[/\\]JUPITER\.md$/);
  });
