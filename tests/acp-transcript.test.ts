...
describe("acp --transcript", () => {
  ...
  beforeAll(() => {
    tmpDir = mkdtempSync(join(tmpdir(), "jupiter-acp-transcript-"));
  });
  ...
  it("writes _meta with source 'jupiter acp' and records the ACP session sequence", async () => {
    ...
    expect(meta?.source).toBe("jupiter acp");
