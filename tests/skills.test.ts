...
function writeSkillDir(
  ...
  const parent =
    which === "global"
      ? join(homeOrProject, ".jupiter", "skills")
      : which === "project"
        ? join(root, ".jupiter", "skills")
        : homeOrProject;
...
function writeFlatSkill(
  ...
  const skills = join(dir, ".jupiter", "skills");
...
describe("SkillStore", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "jupiter-skills-home-"));
    projectRoot = mkdtempSync(join(tmpdir(), "jupiter-skills-proj-"));
  });
  ...
  it("surfaces project-scope skills from <projectRoot>/.jupiter/skills", () => {
    writeSkillDir(
      projectRoot,
      "project",
      "deploy",
  ...
  it("skips dotfiles that would masquerade as skills", () => {
    ...
    const dotDir = join(home, ".jupiter", "skills");
  ...
  it("reads custom flat and dir-layout skills directly from configured roots", () => {
    const custom = mkdtempSync(join(tmpdir(), "jupiter-skills-custom-"));
    try {
      writeSkillDir(
        projectRoot,
        "custom",
  ...
  it("loads skills from symlinked directories (#2104)", () => {
    ...
    const realDir = mkdtempSync(join(tmpdir(), "jupiter-skills-real-"));
    try {
      writeFileSync(
      ...
      const skillsDir = join(home, ".jupiter", "skills");
...
describe("applySkillsIndex", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "jupiter-skills-idx-"));
    projectRoot = mkdtempSync(join(tmpdir(), "jupiter-skills-idx-proj-"));
  });
...
describe("Skill frontmatter — runAs", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "jupiter-skills-runas-"));
  });
  ...
  describe("Claude SKILL.md aliases", () => {
    ...
    it("reads skills from <project>/.claude/skills/", () => {
      const project = mkdtempSync(join(tmpdir(), "jupiter-skills-proj-"));
      try {
...
describe("Built-in skills", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "jupiter-skills-builtins-"));
  });
