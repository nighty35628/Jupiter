...
describe("config", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.JUPITER_SEARCH;
    // biome-ignore lint/performance/noDelete: same reason
  ...
  afterEach(() => {
    ...
    if (originalSearch === undefined) {
      ...
      process.env.JUPITER_SEARCH = originalSearch;
    }
  ...
  it("round-trips the full JupiterConfig (model, effort, mcp, session, setupCompleted)", () => {
    writeConfig(
      {
  ...
  it("searchEnabled honours JUPITER_SEARCH=off/false/0", () => {
    process.env.JUPITER_SEARCH = "off";
    expect(searchEnabled(path)).toBe(false);
    process.env.JUPITER_SEARCH = "false";
    expect(searchEnabled(path)).toBe(false);
    process.env.JUPITER_SEARCH = "0";
    expect(searchEnabled(path)).toBe(false);
  });

  it("searchEnabled stays true for unrelated env values", () => {
    process.env.JUPITER_SEARCH = "on";
    expect(searchEnabled(path)).toBe(true);
  });
  ...
  it.runIf(process.platform === "win32")(
    ...
    () => {
      addProjectShellAllowed("F:\\Jupiter", "gh", path);
      expect(loadProjectShellAllowed("f:\\jupiter", path)).toContain("gh");
      expect(loadProjectShellAllowed("F:\\JUPITER", path)).toContain("gh");
      // Mutations through any-cased rootDir consolidate onto the original key.
      addProjectShellAllowed("f:\\jupiter", "deploy", path);
      expect(loadProjectShellAllowed("F:\\Jupiter", path)).toEqual(["gh", "deploy"]);
      expect(Object.keys(readConfig(path).projects ?? {})).toEqual(["F:\\Jupiter"]);
      expect(removeProjectShellAllowed("f:\\JUPITER", "gh", path)).toBe(true);
      expect(loadProjectShellAllowed("F:\\Jupiter", path)).toEqual(["deploy"]);
      expect(clearProjectShellAllowed("F:\\JUPITER", path)).toBe(1);
      expect(loadProjectShellAllowed("F:\\Jupiter", path)).toEqual([]);
    },
  );
  ...
  it("round-trips semantic embedding config", () => {
    ...
    expect(loaded.openaiCompat?.extraBody).toEqual({ user: "jupiter" });
  });
