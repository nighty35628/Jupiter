/** Bare `jupiter` routing — defaults to code mode in the current directory; explicit `chat` stays chat. */
...
describe("bare CLI routing", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "jupiter-cli-home-"));
    ...
    cwd = realpathSync(mkdtempSync(join(tmpdir(), "jupiter-cli-cwd-")));
  ...
  it("routes bare jupiter to code mode rooted at cwd", async () => {
    writeConfig({ setupCompleted: true }, join(home, ".jupiter", "config.json"));
  ...
  it("routes bare jupiter in a non-project directory to code mode too", async () => {
    writeConfig({ setupCompleted: true }, join(home, ".jupiter", "config.json"));
  ...
  it("keeps explicit jupiter chat in chat mode even inside a project", async () => {
    writeConfig({ setupCompleted: true }, join(home, ".jupiter", "config.json"));
  ...
  it("keeps first-run bare jupiter on the setup wizard", async () => {
    writeConfig({ setupCompleted: false }, join(home, ".jupiter", "config.json"));
    mkdirSync(join(cwd, ".git"));
