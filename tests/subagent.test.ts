...
import { listSessions, loadSessionMeta } from "../src/memory/session.js";
...
describe("registerSubagentTool", () => {
  ...
  it("tags newly spawned subagent sessions with the parent workspace", async () => {
    const home = mkdtempSync(join(tmpdir(), "jupiter-subagent-home-"));
    const workspace = mkdtempSync(join(tmpdir(), "jupiter-subagent-ws-"));
    ...
    try {
      ...
      registerSubagentTool(parent, { client, sink, projectRoot: workspace });

      await parent.dispatch("spawn_subagent", JSON.stringify({ task: "tag workspace" }));

      const runId = events.find((e) => e.kind === "start")?.runId;
      expect(runId).toBeTruthy();
      const createdName = listSessions().find((s) => s.name.startsWith(`subagent-${runId}-`))?.name;
      expect(createdName).toBeTruthy();
      const created = loadSessionMeta(createdName!);
