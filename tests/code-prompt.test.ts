...
describe("codeSystemPrompt", () => {
  let root: string;
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "jupiter-prompt-"));
  });
  afterEach(() => {
  ...
  it("locks identity to this prompt — workspace files don't redefine the assistant", () => {
    ...
    expect(CODE_SYSTEM_PROMPT).toMatch(/developed by nightytech/);
    expect(CODE_SYSTEM_PROMPT).toMatch(/nightytech's coding assistant/);
    expect(CODE_SYSTEM_PROMPT).toMatch(/do not volunteer the current workspace language/);
