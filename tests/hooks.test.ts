...
function writeSettings(dir: string, json: unknown): string {
  const path = join(dir, ".jupiter", "settings.json");
  mkdirSync(join(dir, ".jupiter"), { recursive: true });
...
describe("loadHooks", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "jupiter-hooks-home-"));
    project = mkdtempSync(join(tmpdir(), "jupiter-hooks-proj-"));
  ...
  it("tolerates malformed JSON without throwing", () => {
    ...
    writeFileSync(join(home, ".jupiter", "settings.json"), "{ not valid json", "utf8");
  ...
  it("paths reported by *SettingsPath helpers are absolute", () => {
    expect(globalSettingsPath(home)).toBe(join(home, ".jupiter", "settings.json"));
    expect(projectSettingsPath(project)).toBe(join(project, ".jupiter", "settings.json"));
  });
});
