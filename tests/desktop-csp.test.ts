// Regression test for issue #2020 — desktop CSP must allow DeepSeek API without
// keeping inherited upstream updater endpoints.
...
describe("desktop CSP (tauri.conf.json)", () => {
  ...
  it("does not allow the inherited Jupiter updater R2 endpoint in connect-src", () => {
    expect(csp).not.toContain("https://pub-147fb53b9c1e4bbf891a257968619ea7.r2.dev");
  });

  it("does not allow the inherited updater GitHub fallback in connect-src", () => {
    expect(csp).not.toContain("https://github.com");
  });
