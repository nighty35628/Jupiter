...
describe("registerWebTools", () => {
  ...
  it("web_search can dispatch through Ollama cloud", async () => {
    const configPath = "/tmp/jupiter-web-tools-ollama-search-test.json";
    ...
    globalThis.fetch = vi.fn(async (_url, init) => {
      ...
      return new Response(
        JSON.stringify({
          results: [{ title: "Jupiter", url: "https://example.com/r", content: "Result body" }],
        }),
        { status: 200, headers: { "Content-Type": "application/json" } },
      );
    }) as unknown as typeof fetch;
    try {
      ...
      const out = await registry.dispatch(
        "web_search",
        JSON.stringify({ query: "jupiter", topK: 2 }),
      );
  ...
  it("web_fetch can dispatch through Ollama cloud when selected", async () => {
    const configPath = "/tmp/jupiter-web-tools-ollama-fetch-test.json";
    writeConfig({ webSearchEngine: "ollama", ollamaApiKey: "ollama-test-key" }, configPath);
    const originalFetch = globalThis.fetch;
    globalThis.fetch = vi.fn(async (_url, init) => {
...
describe("webFetch", () => {
  ...
  it("refuses DNS names that resolve to internal addresses without a second lookup", async () => {
