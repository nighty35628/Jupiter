/** `~/.jupiter/memory/` store + prefix-loading composer — temp homeDir per test. */
...
describe("user-memory", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "jupiter-umem-home-"));
    projectRoot = mkdtempSync(join(tmpdir(), "jupiter-umem-proj-"));
  ...
  describe("applyUserMemory", () => {
    ...
    it("respects JUPITER_MEMORY=off", () => {
      const store = new MemoryStore({ homeDir: home, projectRoot });
      store.write({
        name: "pref_one",
        type: "user",
  ...
  describe("applyMemoryStack", () => {
    it("composes JUPITER.md → global memory → project memory", () => {
      writeFileSync(join(projectRoot, "JUPITER.md"), "Pinned by JUPITER.md\n", "utf8");
      // applyMemoryStack uses ~/.jupiter by default — redirect via HOME
      ...
      // Order: JUPITER.md content → global → project. Each unique
      // string should appear, and in that order.
      const iJupiter = out.indexOf("Pinned by JUPITER.md");
      const iGlobal = out.indexOf("g_pref");
      const iProject = out.indexOf("p_fact");
      expect(iJupiter).toBeGreaterThan(BASE.length - 1);
      expect(iGlobal).toBeGreaterThan(iJupiter);
    ...
    it("applyMemoryStack injects no memory blocks when no memory is set", () => {
      ...
      // dev's real ~/.jupiter and bleeds in whatever memory they have.
  ...
  describe("applyGlobalJupiterMemory", () => {
    it("loads ~/.jupiter/JUPITER.md when present", () => {
    ...
    it("returns BASE unchanged when the file is empty / whitespace-only", () => {
      mkdirSync(home, { recursive: true });
      writeFileSync(join(home, "JUPITER.md"), "   \n  \n", "utf8");
      const out = applyGlobalJupiterMemory(BASE, home);
      expect(out).toBe(BASE);
    });

    it("respects JUPITER_MEMORY=off opt-out", () => {
      mkdirSync(home, { recursive: true });
      writeFileSync(join(home, "JUPITER.md"), "- secret\n", "utf8");
      const orig = process.env.JUPITER_MEMORY;
      process.env.JUPITER_MEMORY = "off";
      try {
        const out = applyGlobalJupiterMemory(BASE, home);
        expect(out).toBe(BASE);
        ...
        if (orig === undefined) {
          ...
          process.env.JUPITER_MEMORY = orig;
        }
      }
    });
  });
