...
export class SkillStore {
  ...
  createWithContent(
    ...
    if (scope === "project" && !this.projectRoot) {
      return { error: "project scope requires a workspace — run from `jupiter code`" };
    }
    const root =
      scope === "project"
        ? join(this.projectRoot ?? "", ".jupiter", SKILLS_DIRNAME)
        : join(this.homeDir, ".jupiter", SKILLS_DIRNAME);
...
const BUILTIN_QQ_BODY = `Help the user configure or troubleshoot the built-in QQ channel in Jupiter. This skill is INLINED on purpose — stay in the parent loop and keep the guidance short.
...
Key facts:
- QQ is a remote channel attached to an existing Jupiter session, not a separate mode.
