...
export interface CacheFirstLoopOptions {
  ...
  /** Re-runs the prompt builder (applyMemoryStack / codeSystemPrompt) on /new so JUPITER.md edits take effect without a restart. Accepting a cache miss is the price. */
...
export class CacheFirstLoop {
  ...
  constructor(opts: CacheFirstLoopOptions) {
    ...
    this.repair = new ToolCallRepair({
      ...
      isStormExempt,
      stormThreshold: parsePositiveIntEnv(process.env.JUPITER_STORM_THRESHOLD),
      stormWindow: parsePositiveIntEnv(process.env.JUPITER_STORM_WINDOW),
    });
  ...
  /** "New chat" — drops in-memory messages, archives the on-disk transcript so it survives in Sessions, keeps sessionName so the prefix cache stays warm. Re-runs the system-prompt builder if one was wired (issue #778: JUPITER.md edits otherwise need a restart). */
