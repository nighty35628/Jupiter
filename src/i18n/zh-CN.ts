...
export const zhCN: TranslationSchema = {
  ...
  cli: {
    ...
    version: "打印 Jupiter 版本。",
    update: "检查 Jupiter 自己的更新通道。",
    index: "构建（或增量刷新）本地语义搜索索引。",
  },
  stats: {
    usageHint: "运行 `jupiter chat`、`jupiter code` 或 `jupiter run <task>` — 每次对话都会记录",
    usageDetail: "每次对话在日志中追加一行，`jupiter stats` 会将其汇总统计。",
  },
  run: {
    missingApiKey:
      "未设置 DEEPSEEK_API_KEY 且标准输入不是 TTY（无法交互式输入）。\n" +
      "请设置环境变量，或先运行 `jupiter chat` 交互一次以保存密钥。\n",
  },
  sessions: {
    emptyHint:
      "暂无已保存的会话 — 运行 `jupiter chat`（会话会自动保存，除非使用了 --no-session）。",
    listHeader: "保存的会话 (~/.jupiter/sessions/)：",
    inspectHint: "查看：jupiter sessions <name>",
    resumeHint: "恢复：jupiter chat --session <name>",
  ...
  ui: {
    welcome: "随时运行 `jupiter` 开始聊天 — 您的设置将被记住。",
    ...
    noConfigHint: "本次运行忽略 ~/.jupiter/config.json",
  ...
  code: {
    workspaceConflict:
      "⚠ 工作区包含另一个智能体平台的文件 ({platforms})。Jupiter 可能会将其作为项目内容读取；如果不是您想要的，请使用 --dir <your-project> 重新启动。\n",
  ...
  slash: {
    ...
    memory: {
      description: "显示 / 管理固定记忆（JUPITER.md + ~/.jupiter/memory）",
    ...
    permissions: {
      description: "显示 / 编辑 shell 允许列表（内置只读 · 项目级：~/.jupiter/config.json）",
    ...
    setup: { description: "提醒您退出并运行 `jupiter setup`" },
    semantic: {
    ...
    init: {
      description:
        "扫描项目并合成基线 JUPITER.md（模型写入；使用 /apply 审查）。`force` 覆盖已有文件。",
      argsHint: "[force]",
    },
    apply: {
      description:
  ...
  wizard: {
    ...
    reviewLabelTheme: "主题",
    mcpTitle: "Jupiter 要为你接入哪些 MCP 服务器？",
  ...
  themePicker: {
    ...
    currentPref: "当前偏好",
    activeNow: "当前生效",
    autoDesc: "使用 JUPITER_THEME 或默认主题",
  },
  planFlow: {
  ...
  errors: {
    contextOverflow:
      "上下文溢出（DeepSeek 400）：会话历史已达 {requested}，超出模型 prompt 上限（V4：1M tokens；旧版 chat/reasoner：131k）。通常是单个工具结果太大。Jupiter 默认将新工具结果限制在 8k tokens，并在会话加载时自动修复超大历史 — 重启常能清掉。如果仍然溢出，运行 /new 重新开始，或打开 /sessions 选中后按 [d] 删除该会话。",
    ...
    concurrency429:
      "DeepSeek 并发超限（429）：{inner}。账号在跑的请求超过上限（v4-pro 500、v4-flash 2500，账号下所有 API key 累加）。通常是同一账号开了多个 Jupiter 进程，或者并行 subagent 一次发太多。等几秒重试、减少并行，或在 https://platform.deepseek.com 申请扩容。",
    deepseek5xxHead:
      "DeepSeek 服务不可用（{status}） — 这是 DeepSeek 服务端问题，不是 Jupiter 故障。已按指数退避重试 4 次。",
  ...
  handlers: {
    basic: {
      ...
      newInfoSystemReloaded: " · JUPITER.md / 项目记忆已重新加载（下一轮一次性 cache miss）",
      helpTitle: "命令：",
      ...
      helpMemoryPin: "  #<note>                  将 <note> 追加到 <project>/JUPITER.md（可提交）。",
      helpMemoryPinEx: "                             示例：#findByEmail 必须区分大小写",
      helpMemoryGlobal:
        "  #g <note>                将 <note> 追加到 ~/.jupiter/JUPITER.md（全局，不提交）。",
      ...
      helpSessionCustom: "  jupiter chat --session <name>   使用不同的命名会话",
      helpSessionNone: "  jupiter chat --no-session       禁用本次运行的持久化",
      ...
      aboutHeader: "Jupiter v{version} — 缓存优先的 DeepSeek 编码代理",
    ...
    admin: {
      ...
      hooksProjectFallback: "  · <project>/.jupiter/settings.json（项目）",
      hooksGlobal: "  · {path}（全局）",
      ...
      updateCurrent: "当前：Jupiter {version}",
      updateLatestPending: "最新：（Jupiter 更新通道尚未配置或离线）",
      updateRetryHint: "配置 JUPITER_REGISTRY_URL 或 JUPITER_NPM_PACKAGE 后即可启用检查，",
      updateRetryHint2: "等自己的更新通道就绪后，也可以在另一个终端运行 Jupiter updater。",
      updateLatest: "最新：Jupiter {version}",
      updateUpToDate: "您已是最新版本。无需操作。",
      updateNpxHint: "您正在通过 npx 运行 — 下次 `npx jupiter ...` 启动时将自动获取。",
      updateNpxForce: "要强制刷新：`npm cache clean --force`。",
      updateUpgradeHint: "要升级，请退出此会话并运行：",
      updateUpgradeCmd1: "  jupiter update            （交互式，支持 --dry-run 预览）",
    ...
    edits: {
      undoCodeOnly: "/undo 仅在 `jupiter code` 中可用 — 聊天模式不应用编辑。",
      historyCodeOnly: "/history 仅在 `jupiter code` 中可用。",
      showCodeOnly: "/show 仅在 `jupiter code` 中可用。",
      applyCodeOnly: "/apply 仅在 `jupiter code` 中可用（此处无内容可应用）。",
      discardCodeOnly: "/discard 仅在 `jupiter code` 中可用。",
      planCodeOnly: "/plan 仅在 `jupiter code` 中可用 — 聊天模式不限制工具写入。",
      planOn:
      ...
      modeCodeOnly: "/mode 仅在 `jupiter code` 中可用。",
      ...
      commitCodeOnly: "/commit 仅在 `jupiter code` 中可用（需要有根的 git 仓库）。",
      commitUsage: '用法：/commit "提交消息"  — 在 {root} 中运行 `git add -A && git commit -m "…"`',
      walkCodeOnly: "/walk 仅在 `jupiter code` 中可用。",
      checkpointCodeOnly: "/checkpoint 仅在 `jupiter code` 中可用 — 聊天模式不应用编辑。",
      checkpointNone:
      ...
      restoreCodeOnly: "/restore 仅在 `jupiter code` 中可用。",
      ...
      cwdCodeOnly: "/cwd 仅在 `jupiter code` 中可用。",
      cwdUsage:
    ...
    model: {
      ...
      budgetNoCap:
        "未设置会话预算 — Jupiter 将持续运行直到您停止。使用以下方式设置：/budget <usd>   （例如 /budget 5）",
    ...
    permissions: {
      mutateCodeOnly:
        "/permissions add / remove / clear 仅在 `jupiter code` 中可用 — 它们编辑项目范围的允许列表（`~/.jupiter/config.json` projects[<root>].shellAllowed）。",
      addUsage:
    ...
    plans: {
      noSession: "未附加会话 — `/plans` 是按会话的。在项目中运行 `jupiter code` 以获取会话。",
      ...
      replayNoSession:
        "未附加会话 — `/replay` 是按会话的。在项目中运行 `jupiter code` 以获取会话。",
      replayNoArchives:
    ...
    jobs: {
      codeOnly: "/jobs 仅在 `jupiter code` 中可用。",
      killCodeOnly: "/kill 仅在 `jupiter code` 中可用。",
      logsCodeOnly: "/logs 仅在 `jupiter code` 中可用。",
      empty:
    ...
    memory: {
      disabled:
        "记忆已禁用（环境变量 JUPITER_MEMORY=off）。取消设置该变量以重新启用 — 此期间不会固定任何 JUPITER.md 或 ~/.jupiter/memory 内容。",
      noRoot:
        "此会话无工作目录 — `/memory` 需要一个根目录来解析 JUPITER.md。（在测试环境中运行？）",
      listEmpty:
        "尚无用户记忆。模型可以调用 `remember` 保存一个，或您可以在 ~/.jupiter/memory/global/ 或项目子目录中手动创建文件。",
      ...
      layerGlobal: "  2. ~/.jupiter/memory/global/ — 您的跨项目私有记忆。",
      layerProjectHash: "  3. ~/.jupiter/memory/<project-hash>/ — 此项目的私有记忆。",
    ...
    mcp: {
      noServers:
        '未附加 MCP 服务器。运行 `jupiter setup` 选择一些，或使用 --mcp "<spec>" 启动。`jupiter mcp list` 显示目录。注：模型发起的 shell 命令按次审批（allow once / allow always / deny），设计上没有「全局放行」开关。',
      toolsLabel: "  工具     {count}",
      resourcesHint: "`/resource` 浏览+读取",
      promptsHint: "`/prompt` 浏览+获取",
      awarenessOnly: "聊天模式目前消耗工具；资源+提示在此展示供了解。",
      catalogHint: "完整目录：`jupiter mcp list` · 深度诊断：`jupiter mcp inspect <spec>`。",
      fallbackServers: "MCP 服务器（{count}）：",
      fallbackTools: "注册表中的工具（{count}）：",
      fallbackChange: "要更改此设置，请退出并运行 `jupiter setup`。",
      usageDisableEnable:
    ...
    init: {
      codeOnly:
        "/init 仅在代码模式下工作（需要文件系统工具）。\n运行 `jupiter code [path]` 启动一个以您要初始化的项目为根的会话，\n然后运行 /init。",
      exists: "▸ JUPITER.md 已存在于 {path}",
      existsForce: "  /init force   从头重新生成（覆盖）",
      existsEdit: "  或手动编辑 — 它只是 markdown。当前文件已",
      existsPinned: "  固定到每次启动的系统提示词中。",
      info: "▸ /init — 模型将扫描项目并合成 JUPITER.md。\n  结果将作为待处理的编辑；完成后会报告发生了什么。使用 /apply 或 /walk 审查。",
    },
    webSearchEngine: {
    ...
    skill: {
      listEmpty: "未找到技能。Jupiter 从以下位置读取技能：",
      listProjectScope:
        "  · <project>/.jupiter/skills/<name>/SKILL.md  （或 <name>.md） — 项目范围",
      listGlobalScope: "  · ~/.jupiter/skills/<name>/SKILL.md  （或 <name>.md） — 全局范围",
      listProjectOnly: "  （项目范围仅在 `jupiter code` 中活跃）",
      ...
      listEmptyNewHint:
        "用 `/skill new <name>` 在项目范围下生成一个空白模板，或用 /skill update 刷新内置技能包。",
      showUsage: "用法：/skill show <name>",
      showNotFound: "未找到技能：{name}",
      runNotFound: "未找到技能：{name}  （尝试 /skill list）",
      runInfo: "▸ 正在运行技能：{name}{args}",
      updateChecking: "▸ 正在检查技能包更新渠道：{url}",
      updateEmpty: "技能包更新渠道暂时没有发布任何技能包。",
      updateNoUpdates: "技能包已是最新。",
      updateAvailable: "发现 {count} 个技能包更新：",
      updateInstalled: "已安装 {count} 个技能包更新：",
      updateFailed: "▲ 技能包更新失败：{reason}",
      updateRestartHint: "当前会话的系统提示词不会自动变化；运行 /new 或新建会话后会刷新技能索引。",
  ...
  pathConfirm: {
    ...
    allowAlways: "始终允许",
    allowAlwaysDesc: "记住 `{prefix}`，本项目永久允许（写入 ~/.jupiter/config.json）",
    deny: "拒绝",
  ...
  startup: {
    codeRooted: '▸ jupiter code：根目录 {rootDir}，会话 "{session}" · {tools} 个原生工具{semantic}',
    ephemeral: "（临时）",
    semanticOn: " · 语义搜索已开启",
  },
  doctorErrors: {
  ...
  webErrors: {
    ...
    baiduMissingKey:
      "web_search: 百度 AI Search 需要 API 密钥 — 设置 BAIDU_API_KEY 或 QIANFAN_API_KEY 环境变量，在 ~/.jupiter/config.json 配置 `baiduApiKey`，或运行 /search-engine baidu <key>；可在百度智能云千帆获取密钥",
    baiduUnauthorized:
    ...
    tavilyMissingKey:
      "web_search: Tavily 后端需要 API 密钥 — 设置 TAVILY_API_KEY 环境变量，或在 ~/.jupiter/config.json 中配置 `tavilyApiKey`；https://tavily.com 每月 1000 次免费",
    tavilyUnauthorized:
    ...
    perplexityMissingKey:
      "web_search: Perplexity 后端需要 API 密钥 — 设置 PERPLEXITY_API_KEY 环境变量，或在 ~/.jupiter/config.json 中配置 `perplexityApiKey`；在 https://perplexity.ai/settings/api 获取密钥",
    ...
    exaMissingKey:
      "web_search: Exa 后端需要 API 密钥 — 设置 EXA_API_KEY 环境变量，或在 ~/.jupiter/config.json 中配置 `exaApiKey`；https://exa.ai 每月 1000 次免费",
    exaUnauthorized:
    ...
    ollamaMissingKey:
      "Ollama 需要 API 密钥 — 设置 OLLAMA_API_KEY 环境变量，或在 ~/.jupiter/config.json 中配置 `ollamaApiKey`；在 https://ollama.com/settings/keys 获取密钥",
    ollamaUnauthorized:
    ...
    fetchOllamaMissingKey:
      "web_fetch: Ollama 抓取需要 API 密钥 — 设置 OLLAMA_API_KEY 环境变量，或在 ~/.jupiter/config.json 中配置 `ollamaApiKey`；在 https://ollama.com/settings/keys 获取密钥",
  ...
  mcpMarketplace: {
    ...
    bridgeReloadFailed: "✓ 已安装 {name} — 重启 `jupiter code` 以桥接（重载失败：{message}）",
    restartBridge: "✓ 已安装 {name} — 重启 `jupiter code` 以桥接",
    needsEnv: "  ·  需要环境变量：{env}",
    badgeOfficial: "[官方]",
    badgeSmithery: "[三方]",
    badgeLocal: "[本地]",
  ...
  mcpBrowser: {
    title: "◈ MCP 浏览器",
    empty: "没有挂载 MCP 服务器。运行 `jupiter setup` 选择一些，或使用 --mcp 启动。",
  ...
  mcpLifecycle: {
    ...
    failedSetupHint: "→ 运行 `jupiter setup` 移除此条目，或修复底层问题（缺少 npm 包、网络等）。",
    failedSetupConfigHint: "→ 运行 `jupiter setup` 从已保存配置中移除损坏的条目。",
  ...
  mcpCli: {
    ...
    moreAvailable: "还有更多",
    allLoaded: "已全部加载",
    morePagesAvailable: "▸ 还有更多页可用 — `jupiter mcp list --pages <n>` 或 --all",
    installHint: "安装：jupiter mcp install <name>",
    usageSearch: "用法：jupiter mcp search <query>",
    usageInstall: "用法：jupiter mcp install <name>",
    noMatchesFor: '未找到 "{q}" 的匹配项（已检索 {count} 条记录，来源：{source}）',
    matchCount: '在 {source} 中找到 {count} 条 "{q}" 的匹配项（已扫描 {loaded} 条记录）：',
    moreLoaded: "… 还有 {count} 条已加载 — 使用 `jupiter mcp search <query>` 筛选",
    moreMatches: "… 还有 {count} 条匹配项",
    installed: "已安装：{spec}",
    noServerFound: '在 {source} 中遍历了 {pages} 页后未找到名为 "{target}" 的 MCP 服务器。',
    noServerTryMore: "试试：jupiter mcp install {target} --max-pages 100",
