...
export const ru: TranslationSchema = {
  ...
  cli: {
    ...
    version: "Вывести версию Jupiter.",
    update: "Проверить новую версию Jupiter и установить.",
    index: "Построить (или инкрементально обновить) локальный семантический поисковый индекс.",
  },
  stats: {
    ...EN.stats,
    usageHint: "запусти `jupiter chat`, `jupiter code` или `jupiter run <task>` — каждый шаг",
    usageDetail: "добавляет одну строку в лог, а `jupiter stats` сводит всё вместе.",
  },
  run: {
    ...EN.run,
    missingApiKey:
      "DEEPSEEK_API_KEY не задан и stdin не TTY (нельзя запросить).\n" +
      "Установи переменную окружения или запусти `jupiter chat` один раз для сохранения ключа.\n",
  },
  sessions: {
    ...EN.sessions,
    emptyHint:
      "ещё нет сохранённых сессий — запусти `jupiter chat` (сессии авто-сохраняются, если не указан --no-session).",
    listHeader: "Сохранённые сессии (~/.jupiter/sessions/):",
    inspectHint: "Просмотр:  jupiter sessions <имя>",
    resumeHint: "Возобновить: jupiter chat --session <имя>",
  ...
  ui: {
    ...
    modelIdHint: "ID модели DeepSeek (напр. deepseek-flash)",
    ...
    noConfigHint: "игнорировать ~/.jupiter/config.json для этого запуска",
    ...
    modelOverrideFlash: "переопределить модель (по умолч.: deepseek-flash)",
    skipConfirmHint: "пропустить запрос подтверждения",
    welcome: "Запускай `jupiter` в любое время — настройки сохраняются.",
  ...
  code: {
    ...EN.code,
    workspaceConflict:
      "⚠ рабочая область содержит файлы другой платформы агента ({platforms}). Jupiter может прочитать их как содержимое проекта; перезапусти с --dir <твой-проект> если это нежелательно.\n",
  ...
  slash: {
    ...
    memory: {
      ...EN.slash.memory,
      description: "показать / управлять закреплённой памятью (JUPITER.md + ~/.jupiter/memory)",
    ...
    hooks: {
      ...EN.slash.hooks,
      description:
        "список активных хуков (settings.json в .jupiter/) · reload перечитывает с диска",
    },
    permissions: {
      ...EN.slash.permissions,
      description:
        "показать / редактировать белый список команд (встроенные только для чтения · на проект: ~/.jupiter/config.json)",
    },
    dashboard: {
      ...EN.slash.dashboard,
    ...
    setup: { ...EN.slash.setup, description: "напоминает выйти и запустить `jupiter setup`" },
    semantic: {
      ...EN.slash.semantic,
    ...
    init: {
      ...EN.slash.init,
      description:
        "просканировать проект и синтезировать базовый JUPITER.md (модель пишет; просмотр через /apply). `force` перезаписывает существующий файл.",
    },
    apply: {
      ...EN.slash.apply,
      description:
  ...
  wizard: {
    ...
    welcomeTitle: "Добро пожаловать в Jupiter.",
    ...
    mcpTitle: "Какие MCP-серверы должен подключить Jupiter?",
  ...
  themePicker: {
    ...
    autoDesc: "использовать JUPITER_THEME или по умолчанию",
  },
  planFlow: {
    ...EN.planFlow,
  ...
  webErrors: {
    ...EN.webErrors,
    braveMissingKey:
      "web_search: Для работы Brave Search требуется ключ API — задайте переменную среды BRAVE_SEARCH_API_KEY (или BRAVE_API_KEY) или параметр `braveApiKey` в файле ~/.jupiter/config.json; бесплатная регистрация с лимитом 2000 запросов в месяц на сайте https://brave.com/search/api/",
    braveUnauthorized:
    ...
    ollamaMissingKey:
      "web_search: Для Ollama требуется ключ API — задайте переменную среды OLLAMA_API_KEY или параметр `ollamaApiKey` в ~/.jupiter/config.json; получить ключ можно на https://ollama.com/settings/keys",
    ollamaUnauthorized:
