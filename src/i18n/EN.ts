...
export const EN: TranslationSchema = {
  ...
  cli: {
    ...
    version: "Print Jupiter version.",
    update: "Check Jupiter's own update channel.",
    index: "Build (or incrementally refresh) a local semantic search index.",
  },
  stats: {
    usageHint: "run `jupiter chat`, `jupiter code`, or `jupiter run <task>` — every turn",
    usageDetail: "appends one line to the log and `jupiter stats` will roll it up.",
  },
  run: {
    missingApiKey:
      "DEEPSEEK_API_KEY is not set and stdin is not a TTY (cannot prompt).\n" +
      "Set the env var, or run `jupiter chat` once interactively to save a key.\n",
  },
  sessions: {
    emptyHint:
      "no saved sessions yet — run `jupiter chat` (sessions are auto-saved unless --no-session).",
    listHeader: "Saved sessions (~/.jupiter/sessions/):",
    inspectHint: "Inspect:  jupiter sessions <name>",
    resumeHint: "Resume:   jupiter chat --session <name>",
  ...
  ui: {
    welcome: "Run `jupiter` any time to start chatting — your settings are remembered.",
    ...
    keysReference: {
      topic: "Jupiter keys + mouse reference",
      sections: [
        {
          title: "keyboard",
          rows: [
    ...
    noConfigHint: "ignore ~/.jupiter/config.json for this run",
  ...
  code: {
    workspaceConflict:
      "⚠ workspace contains another agent platform's files ({platforms}). Jupiter may read them as project content; relaunch with --dir <your-project> if that's not what you want.\n",
  ...
  slash: {
    ...
    memory: {
      description: "show / manage pinned memory (JUPITER.md + ~/.jupiter/memory)",
    ...
    setup: { description: "reminds you to exit and run `jupiter setup`" },
    semantic: {
  ...
  wizard: {
    ...
    welcomeTitle: "Welcome to Jupiter.",
    ...
    mcpTitle: "Which MCP servers should Jupiter wire up for you?",
  ...
  themePicker: {
    ...
    autoDesc: "use JUPITER_THEME or default",
  },
  planFlow: {
  ...
  errors: {
    ...
    concurrency429:
      "DeepSeek concurrency limit hit (429): {inner}. The account has too many in-flight requests (cap: 500 for v4-pro, 2500 for v4-flash, summed across API keys account-wide). Usually means another Jupiter process is sharing the same key, or a parallel subagent fan-out overshot. Wait a few seconds and retry, reduce parallelism, or request a higher cap at https://platform.deepseek.com.",
    deepseek5xxHead:
      "DeepSeek service unavailable ({status}) — this is a DeepSeek-side problem, not Jupiter. Already retried 4× with backoff.",
  ...
  handlers: {
    basic: {
      ...
      newInfoSystemReloaded:
        " · JUPITER.md / project memory reloaded (next turn pays one cache miss)",
      helpTitle: "Commands:",
      ...
      helpMemoryPin:
        "  #<note>                  append <note> to <project>/JUPITER.md (committable).",
      helpMemoryPinEx:
        "                             Example: #findByEmail must be case-insensitive",
      helpMemoryGlobal:
        "  #g <note>                append <note> to ~/.jupiter/JUPITER.md (global, never committed).",
      ...
      helpSessionCustom: "  jupiter chat --session <name>   use a different named session",
      helpSessionNone: "  jupiter chat --no-session       disable persistence for this run",
      ...
      aboutHeader: "Jupiter v{version} — a cache-first DeepSeek coding agent",
    ...
    dingtalk: {
      unavailable: "/dingtalk is not available in this session.",
      connecting: "DingTalk: connecting…",
      connectFailed: "DingTalk connect failed: {reason}",
      disconnecting: "DingTalk: disconnecting…",
      disconnectFailed: "DingTalk disconnect failed: {reason}",
      usage:
        "Usage: /dingtalk connect [clientId clientSecret [mention|all]] | /dingtalk status | /dingtalk disconnect",
      credentialsRequired:
        "DingTalk Client ID and Client Secret are required. Pass them to `/dingtalk connect` or configure them in settings.",
      connected: "DingTalk connected in {mode} mode. It will auto-start on future launches.",
      alreadyConnected: "DingTalk is already connected in {mode} mode. Auto-start is enabled.",
      disconnected: "DingTalk disconnected. Auto-start is disabled.",
      status:
        "DingTalk: {connected}, auto-start {enabled}, credentials {configured}, clientId {clientId}, group policy {groupPolicy}, current mode {mode}.",
      stateConnected: "connected",
      stateDisconnected: "disconnected",
      stateEnabled: "enabled",
      stateDisabled: "disabled",
    ...
    admin: {
      ...
      hooksProjectFallback: "  · <project>/.jupiter/settings.json (project)",
      ...
      updateCurrent: "current: Jupiter {version}",
      updateLatestPending: "latest:  (Jupiter update channel is not configured or offline)",
      updateRetryHint: "configure JUPITER_REGISTRY_URL or JUPITER_NPM_PACKAGE to enable checks,",
      updateRetryHint2:
        "or run the Jupiter updater from another terminal once your channel exists.",
      updateLatest: "latest:  Jupiter {version}",
      updateUpToDate: "you're on the latest. nothing to do.",
      updateNpxHint: "you're running via npx — the next `npx jupiter ...` launch will auto-fetch.",
      updateNpxForce: "to force a refresh sooner: `npm cache clean --force`.",
      updateUpgradeHint: "to upgrade, exit this session and run:",
      updateUpgradeCmd1:
        "  jupiter update            (interactive, dry-run supported via --dry-run)",
    ...
    edits: {
      undoCodeOnly:
        "/undo is only available inside `jupiter code` — chat mode doesn't apply edits.",
      historyCodeOnly: "/history is only available inside `jupiter code`.",
      showCodeOnly: "/show is only available inside `jupiter code`.",
      applyCodeOnly: "/apply is only available inside `jupiter code` (nothing to apply here).",
      discardCodeOnly: "/discard is only available inside `jupiter code`.",
      planCodeOnly:
        "/plan is only available inside `jupiter code` — chat mode doesn't gate tool writes.",
      planOn:
      ...
      modeCodeOnly: "/mode is only available inside `jupiter code`.",
      ...
      commitCodeOnly: "/commit is only available inside `jupiter code` (needs a rooted git repo).",
      commitUsage:
        'usage: /commit "your commit message"  — runs `git add -A && git commit -m "…"` in {root}',
      walkCodeOnly: "/walk is only available inside `jupiter code`.",
      checkpointCodeOnly:
        "/checkpoint is only available inside `jupiter code` — chat mode doesn't apply edits.",
      checkpointNone:
      ...
      restoreCodeOnly: "/restore is only available inside `jupiter code`.",
      ...
      cwdCodeOnly: "/cwd is only available inside `jupiter code`.",
      cwdUsage:
    ...
    model: {
      ...
      budgetNoCap:
        "no session budget set — Jupiter will keep going until you stop it. Set one with: /budget <usd>   (e.g. /budget 5)",
      budgetStatus:
    ...
    permissions: {
      mutateCodeOnly:
        "/permissions add / remove / clear are only available inside `jupiter code` — they edit the project-scoped allowlist (`~/.jupiter/config.json` projects[<root>].shellAllowed).",
      addUsage:
    ...
    plans: {
      noSession:
        "no session attached — `/plans` is per-session. Run `jupiter code` in a project to get a session.",
      ...
      replayNoSession:
        "no session attached — `/replay` is per-session. Run `jupiter code` in a project to get a session.",
      replayNoArchives:
    ...
    jobs: {
      codeOnly: "/jobs is only available inside `jupiter code`.",
      killCodeOnly: "/kill is only available inside `jupiter code`.",
      logsCodeOnly: "/logs is only available inside `jupiter code`.",
      empty:
    ...
    memory: {
      disabled:
        "memory is disabled (JUPITER_MEMORY=off in env). Unset the var to re-enable — no JUPITER.md or ~/.jupiter/memory content will be pinned in the meantime.",
      noRoot:
        "no working directory on this session — `/memory` needs a root to resolve JUPITER.md from. (Running in a test harness?)",
      listEmpty:
        "no user memories yet. The model can call `remember` to save one, or you can create files by hand in ~/.jupiter/memory/global/ or the per-project subdir.",
      ...
      layerGlobal: "  2. ~/.jupiter/memory/global/ — your cross-project private memory.",
      layerProjectHash: "  3. ~/.jupiter/memory/<project-hash>/ — this project's private memory.",
    ...
    mcp: {
      noServers:
        'no MCP servers attached. Run `jupiter setup` to pick some, or launch with --mcp "<spec>". `jupiter mcp list` shows the catalog. Note: model-invoked shell commands are gated per-call (allow once / allow always / deny) — no global allow-all flag.',
      ...
      catalogHint:
        "Full catalog: `jupiter mcp list` · deeper diagnosis: `jupiter mcp inspect <spec>`.",
      fallbackServers: "MCP servers ({count}):",
      fallbackTools: "Tools in registry ({count}):",
      fallbackChange: "To change this set, exit and run `jupiter setup`.",
      usageDisableEnable:
    ...
    init: {
      ...
      info: "▸ /init — model will scan the project and synthesize JUPITER.md.\n  The result lands as a pending edit; it will report what happened when finished. Review with /apply or /walk.",
    },
    webSearchEngine: {
    ...
    skill: {
      listEmpty: "no skills found. Jupiter reads skills from:",
      listProjectScope:
        "  · <project>/.jupiter/skills/<name>/SKILL.md  (or <name>.md)  — project scope",
      listGlobalScope: "  · ~/.jupiter/skills/<name>/SKILL.md  (or <name>.md)  — global scope",
      listProjectOnly: "  (project scope is only active in `jupiter code`)",
      ...
      listEmptyNewHint:
        "Scaffold one with: /skill new <name>  (project scope), or refresh built-in packs with /skill update.",
      ...
      updateChecking: "▸ checking skill pack update channel: {url}",
      updateEmpty: "No skill packs are published by the update channel yet.",
      updateNoUpdates: "Skill packs are up to date.",
      updateAvailable: "Skill pack update(s) available: {count}",
      updateInstalled: "Installed skill pack update(s): {count}",
      updateFailed: "▲ skill pack update failed: {reason}",
      updateRestartHint:
  ...
  pathConfirm: {
    ...
    allowAlwaysDesc: "remember `{prefix}` for this project (persisted in ~/.jupiter/config.json)",
    deny: "deny",
  ...
  workspacePicker: {
    header: " ◈ JUPITER · pick a workspace ",
  ...
  startup: {
    codeRooted:
      '\u25b8 jupiter code: rooted at {rootDir}, session "{session}" \u00b7 {tools} native tool(s){semantic}',
    ephemeral: "(ephemeral)",
  ...
  webErrors: {
    ...
    baiduMissingKey:
      "web_search: Baidu AI Search requires an API key \u2014 set BAIDU_API_KEY or QIANFAN_API_KEY env var, configure `baiduApiKey` in ~/.jupiter/config.json, or run /search-engine baidu <key>. Get one from Baidu Cloud Qianfan.",
    baiduUnauthorized:
    ...
    tavilyMissingKey:
      "web_search: Tavily backend requires an API key \u2014 set TAVILY_API_KEY env var or `tavilyApiKey` in ~/.jupiter/config.json; free 1000/mo signup at https://tavily.com",
    tavilyUnauthorized:
    ...
    perplexityMissingKey:
      "web_search: Perplexity backend requires an API key \u2014 set PERPLEXITY_API_KEY env var or `perplexityApiKey` in ~/.jupiter/config.json; get one at https://perplexity.ai/settings/api",
    ...
    exaMissingKey:
      "web_search: Exa backend requires an API key \u2014 set EXA_API_KEY env var or `exaApiKey` in ~/.jupiter/config.json; free 1000/mo signup at https://exa.ai",
    exaUnauthorized:
    ...
    braveMissingKey:
      "web_search: Brave Search requires an API key \u2014 set BRAVE_SEARCH_API_KEY (or BRAVE_API_KEY) env var or `braveApiKey` in ~/.jupiter/config.json; free 2000/mo signup at https://brave.com/search/api/",
    braveUnauthorized:
    ...
    ollamaMissingKey:
      "Ollama requires an API key \u2014 set OLLAMA_API_KEY env var or `ollamaApiKey` in ~/.jupiter/config.json; get one at https://ollama.com/settings/keys",
    ollamaUnauthorized:
    ...
    fetchOllamaMissingKey:
      "web_fetch: Ollama fetch requires an API key \u2014 set OLLAMA_API_KEY env var or `ollamaApiKey` in ~/.jupiter/config.json; get one at https://ollama.com/settings/keys",
  ...
  mcpHealth: {
    ...
    emptyHint:
      "\u2139 no MCP servers configured \u2014 try: `jupiter setup` to re-pick, or `jupiter mcp install filesystem` \u00b7 shell commands gate per-call (allow once / allow always / deny), no global allow-all",
  },
  denyContextInput: {
    description:
  ...
  mcpMarketplace: {
    ...
    bridgeReloadFailed:
      "\u2713 installed {name} - restart `jupiter code` to bridge (reload failed: {message})",
    restartBridge: "\u2713 installed {name} - restart `jupiter code` to bridge",
  ...
  mcpBrowser: {
    title: "\u25c8 MCP browser",
    empty: "No MCP servers attached. Run `jupiter setup` to pick some, or launch with --mcp.",
  ...
  mcpLifecycle: {
    ...
    failedSetupHint:
      "→ run `jupiter setup` to remove this entry, or fix the underlying issue (missing npm package, network, etc.).",
    failedSetupConfigHint: "→ run `jupiter setup` to remove broken entries from your saved config.",
    abortedHint:
  ...
  checkpointPicker: {
    ...
    header: " \u25c8 JUPITER \u00b7 pick a checkpoint ",
  ...
  mcpCli: {
    ...
    morePagesAvailable:
      "\u25b8 more pages available \u2014 `jupiter mcp list --pages <n>` or --all",
    installHint: "Install:  jupiter mcp install <name>",
    usageSearch: "usage: jupiter mcp search <query>",
    usageInstall: "usage: jupiter mcp install <name>",
    ...
    moreLoaded: "\u2026 {count} more loaded \u2014 use `jupiter mcp search <query>` to filter",
    moreMatches: "\u2026 {count} more matches",
    installed: "Installed: {spec}",
    noServerFound:
      'No MCP server named "{target}" found after walking {pages} page(s) of the {source} registry.',
    noServerTryMore: "Try: jupiter mcp install {target} --max-pages 100",
    noInstallMeta:
