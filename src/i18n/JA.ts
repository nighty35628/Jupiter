...
export const JA: TranslationSchema = {
  ...
  cli: {
    ...
    version: "Jupiter のバージョンを表示します。",
    update: "Jupiter 独自の更新チャンネルを確認します。",
    index: "ローカルのセマンティック検索インデックスを構築（または増分更新）します。",
  },
  stats: {
    usageHint: "`jupiter chat`、`jupiter code`、または `jupiter run <task>` を実行するたびに",
    usageDetail: "ログに1行追加され、`jupiter stats` で集計されます。",
  },
  run: {
    missingApiKey:
      "DEEPSEEK_API_KEY が設定されておらず、stdin が TTY ではありません（プロンプトを表示できません）。\n" +
      "環境変数を設定するか、`jupiter chat` を一度対話的に実行してキーを保存してください。\n",
  },
  sessions: {
    emptyHint:
      "保存されたセッションはまだありません — `jupiter chat` を実行してください（--no-session を付けない限り自動保存されます）。",
    listHeader: "保存されたセッション (~/.jupiter/sessions/):",
    inspectHint: "詳細表示:  jupiter sessions <name>",
    resumeHint: "再開:   jupiter chat --session <name>",
  ...
  ui: {
    ...
    noConfigHint: "この実行では ~/.jupiter/config.json を無視",
  ...
  code: {
    workspaceConflict:
      "⚠ ワークスペースに別のエージェントプラットフォームのファイル ({platforms}) が含まれています。Jupiter がそれらをプロジェクトコンテンツとして読み取る可能性があります。意図しない場合は --dir <your-project> で再起動してください。\n",
  ...
  slash: {
    ...
    memory: {
      description: "ピン留めメモリの表示/管理 (JUPITER.md + ~/.jupiter/memory)",
    ...
    hooks: {
      description:
        "アクティブなフックを一覧 (.jupiter/ 下の settings.json) · reload でディスクから再読込",
      argsHint: "[reload]",
    },
    permissions: {
      description:
        "シェル許可リストの表示/編集（ビルトインは読取専用 · プロジェクト毎: ~/.jupiter/config.json）",
    ...
    init: {
      description:
        "プロジェクトをスキャンしてベースラインの JUPITER.md を生成（モデルが作成; /apply でレビュー）。`force` で既存ファイルを上書き。",
      argsHint: "[force]",
    },
    apply: {
      description:
  ...
  wizard: {
    ...
    mcpTitle: "Jupiter に接続するMCPサーバーを選択してください",
  ...
  themePicker: {
    ...
    currentPref: "現在の設定",
    activeNow: "現在適用中",
    autoDesc: "JUPITER_THEME またはデフォルトを使用",
  },
  planFlow: {
  ...
  errors: {
    contextOverflow:
      "コンテキストオーバーフロー (DeepSeek 400): セッション履歴が {requested} で、モデルのプロンプト制限（V4: 1Mトークン; 旧来のchat/reasoner: 131k）を超えています。通常、単一のツール結果が大きすぎるのが原因です。Jupiterは新しいツール結果を8kトークンに制限し、セッション読み込み時に大きすぎる履歴を自動修復します — 再起動で解決することが多いです。それでもオーバーフローする場合は、/new で新規開始するか、/sessions を開いて [d] でこのセッションを削除してください。",
    ...
    concurrency429:
      "DeepSeekの同時実行制限に達しました (429): {inner}。アカウントの処理中リクエストが多すぎます（上限: v4-proは500、v4-flashは2500、アカウント全体のAPIキー合計）。通常、別のJupiterプロセスが同じキーを共有しているか、並列サブエージェントのファンアウトが超過したことを意味します。数秒待って再試行するか、並列度を下げるか、https://platform.deepseek.com でより高い上限をリクエストしてください。",
    deepseek5xxHead:
      "DeepSeekサービス利用不可 ({status}) — これはDeepSeek側の問題であり、Jupiterの問題ではありません。バックオフ付きで4回再試行済みです。",
  ...
  handlers: {
    basic: {
      ...
      newInfoSystemReloaded:
        " · JUPITER.md / プロジェクトメモリを再読み込みしました（次のターンでキャッシュミス1回）",
      helpTitle: "コマンド:",
      ...
      helpMemoryPin:
        "  #<note>                  <note> を <project>/JUPITER.md に追記（コミット可能）。",
      helpMemoryPinEx: "                             例: #findByEmail は大文字小文字を区別しない",
      helpMemoryGlobal:
        "  #g <note>                <note> を ~/.jupiter/JUPITER.md に追記（グローバル、コミット不可）。",
      ...
      helpSessionCustom: "  jupiter chat --session <name>   別の名前付きセッションを使用",
      helpSessionNone: "  jupiter chat --no-session       この実行の永続化を無効化",
      retryNone:
      ...
      aboutHeader: "Jupiter v{version} — キャッシュファーストのDeepSeekコーディングエージェント",
    ...
    admin: {
      ...
      hooksProjectFallback: "  · <project>/.jupiter/settings.json（プロジェクト）",
      ...
      updateCurrent: "現在: Jupiter {version}",
      updateLatestPending: "最新:  （Jupiter の更新チャンネルは未設定またはオフライン）",
      updateRetryHint: "JUPITER_REGISTRY_URL または JUPITER_NPM_PACKAGE を設定すると確認できます。",
      updateRetryHint2:
        "独自の更新チャンネルが用意できたら、別の端末で Jupiter updater を実行してください。",
      updateLatest: "最新:  Jupiter {version}",
      updateUpToDate: "最新バージョンです。何もする必要はありません。",
      updateNpxHint: "npx 経由で実行中です — 次回の `npx jupiter ...` 起動時に自動取得されます。",
      updateNpxForce: "より早く強制リフレッシュするには: `npm cache clean --force`。",
      updateUpgradeHint: "アップグレードするには、このセッションを終了して以下を実行してください:",
      updateUpgradeCmd1: "  jupiter update            （対話的、--dry-run でドライラン可能）",
    ...
    edits: {
      undoCodeOnly:
        "/undo は `jupiter code` 内でのみ利用可能です — チャットモードでは編集を適用しません。",
      historyCodeOnly: "/history は `jupiter code` 内でのみ利用可能です。",
      showCodeOnly: "/show は `jupiter code` 内でのみ利用可能です。",
      applyCodeOnly:
        "/apply は `jupiter code` 内でのみ利用可能です（ここで適用するものはありません）。",
      discardCodeOnly: "/discard は `jupiter code` 内でのみ利用可能です。",
      planCodeOnly:
        "/plan は `jupiter code` 内でのみ利用可能です — チャットモードではツール書き込みを制御しません。",
      planOn:
      ...
      modeCodeOnly: "/mode は `jupiter code` 内でのみ利用可能です。",
      ...
      modeReview:
        "▸ 編集モード: review — 編集は /apply（または y）/ /discard（または n）のキューに入ります",
      commitCodeOnly:
        "/commit は `jupiter code` 内でのみ利用可能です（git リポジトリが必要です）。",
      commitUsage:
        '使い方: /commit "コミットメッセージ"  — {root} で `git add -A && git commit -m "…"` を実行します',
      walkCodeOnly: "/walk は `jupiter code` 内でのみ利用可能です。",
      checkpointCodeOnly:
        "/checkpoint は `jupiter code` 内でのみ利用可能です — チャットモードでは編集を適用しません。",
      checkpointNone:
      ...
      restoreCodeOnly: "/restore は `jupiter code` 内でのみ利用可能です。",
      ...
      cwdCodeOnly: "/cwd は `jupiter code` 内でのみ利用可能です。",
      cwdUsage:
    ...
    permissions: {
      mutateCodeOnly:
        "/permissions add / remove / clear は `jupiter code` 内でのみ利用可能です — プロジェクトスコープの許可リストを編集します（`~/.jupiter/config.json` projects[<root>].shellAllowed）。",
      addUsage:
    ...
    plans: {
      noSession:
        "セッションが接続されていません — `/plans` はセッションごとです。プロジェクトで `jupiter code` を実行するとセッションが取得できます。",
      ...
      replayNoSession:
        "セッションが接続されていません — `/replay` はセッションごとです。プロジェクトで `jupiter code` を実行するとセッションが取得できます。",
      replayNoArchives:
    ...
    jobs: {
      codeOnly: "/jobs は `jupiter code` 内でのみ利用可能です。",
      killCodeOnly: "/kill は `jupiter code` 内でのみ利用可能です。",
      logsCodeOnly: "/logs は `jupiter code` 内でのみ利用可能です。",
      empty:
    ...
    memory: {
      disabled:
        "メモリが無効です（JUPITER_MEMORY=off が環境変数に設定されています）。変数を解除して再有効化してください — その間、JUPITER.md や ~/.jupiter/memory の内容はピン留めされません。",
      noRoot:
        "このセッションには作業ディレクトリがありません — `/memory` は JUPITER.md を解決するためのルートが必要です。（テストハーネスで実行中ですか？）",
      listEmpty:
        "まだユーザーメモリがありません。モデルが `remember` を呼び出して保存するか、~/.jupiter/memory/global/ またはプロジェクトごとのサブディレクトリに手動でファイルを作成できます。",
      ...
      layerGlobal: "  2. ~/.jupiter/memory/global/ — プロジェクト横断のプライベートメモリ。",
      layerProjectHash:
        "  3. ~/.jupiter/memory/<project-hash>/ — このプロジェクトのプライベートメモリ。",
    ...
    mcp: {
      noServers:
        'MCPサーバーが接続されていません。`jupiter setup` を実行して選択するか、--mcp "<spec>" で起動してください。`jupiter mcp list` でカタログを表示します。注意: モデル起動のシェルコマンドは呼び出しごとに制御されます（allow once / allow always / deny）— 全許可フラグはありません。',
      toolsLabel: "  tools     {count}",
      resourcesHint: "`/resource` で閲覧+読取",
      promptsHint: "`/prompt` で閲覧+取得",
      awarenessOnly:
        "チャットモードは現在ツールのみ利用; リソースとプロンプトは参照用にここに表示されています。",
      catalogHint: "完全なカタログ: `jupiter mcp list` · 詳細診断: `jupiter mcp inspect <spec>`。",
      fallbackServers: "MCPサーバー ({count}):",
      fallbackTools: "登録済みツール ({count}):",
      fallbackChange: "このセットを変更するには、終了して `jupiter setup` を実行してください。",
      usageDisableEnable:
    ...
    init: {
      codeOnly:
        "/init はコードモードでのみ動作します（ファイルシステムツールが必要です）。\n初期化したいプロジェクトをルートとして `jupiter code [path]` でセッションを開始し、/init を実行してください。",
      exists: "▸ JUPITER.md は既に {path} に存在します",
      ...
      info: "▸ /init — モデルがプロジェクトをスキャンして JUPITER.md を生成します。\n  結果は保留中の編集として届きます; /apply または /walk でレビューしてください。",
    },
    webSearchEngine: {
    ...
    skill: {
      listEmpty: "スキルが見つかりません。Jupiter は以下からスキルを読み取ります:",
      listProjectScope:
        "  · <project>/.jupiter/skills/<name>/SKILL.md  （または <name>.md） — プロジェクトスコープ",
      listGlobalScope:
        "  · ~/.jupiter/skills/<name>/SKILL.md  （または <name>.md） — グローバルスコープ",
      listProjectOnly: "  （プロジェクトスコープは `jupiter code` 内でのみアクティブ）",
      listFrontmatter:
  ...
  pathConfirm: {
    ...
    allowAlways: "常に許可",
    allowAlwaysDesc: "`{prefix}` をこのプロジェクトで記憶（~/.jupiter/config.json に永続化）",
    deny: "拒否",
  ...
  startup: {
    codeRooted:
      '\u25b8 jupiter code: ルート {rootDir}, セッション "{session}" \u00b7 {tools} ネイティブツール{semantic}',
    ephemeral: "（エフェメラル）",
  ...
  webErrors: {
    ...
    baiduMissingKey:
      "web_search: Baidu AI SearchにはAPIキーが必要です \u2014 BAIDU_API_KEY または QIANFAN_API_KEY 環境変数、~/.jupiter/config.json の `baiduApiKey`、または /search-engine baidu <key> を設定してください",
    baiduUnauthorized:
    ...
    tavilyMissingKey:
      "web_search: TavilyバックエンドにはAPIキーが必要です \u2014 TAVILY_API_KEY 環境変数または ~/.jupiter/config.json の `tavilyApiKey` を設定してください。無料1000回/月の登録は https://tavily.com から",
    tavilyUnauthorized:
    ...
    perplexityMissingKey:
      "web_search: PerplexityバックエンドにはAPIキーが必要です \u2014 PERPLEXITY_API_KEY 環境変数または ~/.jupiter/config.json の `perplexityApiKey` を設定してください。https://perplexity.ai/settings/api から取得できます",
    ...
    exaMissingKey:
      "web_search: ExaバックエンドにはAPIキーが必要です \u2014 EXA_API_KEY 環境変数または ~/.jupiter/config.json の `exaApiKey` を設定してください。無料1000回/月の登録は https://exa.ai から",
    exaUnauthorized:
    ...
    braveMissingKey:
      "web_search: Brave SearchにはAPIキーが必要です \u2014 BRAVE_SEARCH_API_KEY（または BRAVE_API_KEY）環境変数、または ~/.jupiter/config.json の `braveApiKey` を設定してください。https://brave.com/search/api/ で月2000回まで無料登録できます",
    braveUnauthorized:
    ...
    ollamaMissingKey:
      "Ollama には API キーが必要です — OLLAMA_API_KEY 環境変数を設定するか、~/.jupiter/config.json に `ollamaApiKey` を設定してください。https://ollama.com/settings/keys で取得できます",
    ollamaUnauthorized:
    ...
    fetchOllamaMissingKey:
      "web_fetch: Ollama 取得には API キーが必要です — OLLAMA_API_KEY 環境変数を設定するか、~/.jupiter/config.json に `ollamaApiKey` を設定してください。https://ollama.com/settings/keys で取得できます",
  ...
  mcpHealth: {
    ...
    emptyHint:
      "\u2139 MCPサーバーが設定されていません \u2014 対処: `jupiter setup` で再選択するか、`jupiter mcp install filesystem` \u00b7 シェルコマンドは毎回確認（allow once / allow always / deny）、全許可フラグなし",
  },
  denyContextInput: {
    description:
  ...
  mcpMarketplace: {
    ...
    bridgeReloadFailed:
      "\u2713 {name} をインストール - `jupiter code` を再起動してブリッジ（再読込失敗: {message}）",
    restartBridge: "\u2713 {name} をインストール - `jupiter code` を再起動してブリッジ",
    needsEnv: "  \u00b7  要環境変数: {env}",
    badgeOfficial: "[公式]",
    badgeSmithery: "[smt]",
    badgeLocal: "[ローカル]",
  ...
  mcpBrowser: {
    title: "\u25c8 MCP ブラウザ",
    empty:
      "MCPサーバーが接続されていません。`jupiter setup` を実行して選択するか、--mcp で起動してください。",
  ...
  mcpLifecycle: {
    ...
    failedSetupHint:
      "→ `jupiter setup` を実行してこのエントリを削除するか、根本的な問題（npmパッケージ不足、ネットワークなど）を修正してください。",
    failedSetupConfigHint:
      "→ `jupiter setup` を実行して、保存された設定から壊れたエントリを削除してください。",
    abortedHint:
  ...
  mcpCli: {
    ...
    allLoaded: "全件読み込み済み",
    morePagesAvailable:
      "\u25b8 さらにページがあります \u2014 `jupiter mcp list --pages <n>` または --all",
    installHint: "インストール:  jupiter mcp install <name>",
    usageSearch: "使い方: jupiter mcp search <query>",
    usageInstall: "使い方: jupiter mcp install <name>",
    noMatchesFor: '{count} 件の読み込み済みエントリ中に "{q}" の一致はありません（{source}）',
    matchCount: '{source} レジストリで "{q}" に {count} 件一致（{loaded} エントリをスキャン）:',
    moreLoaded:
      "\u2026 さらに {count} 件読み込み済み \u2014 `jupiter mcp search <query>` でフィルタしてください",
    moreMatches: "\u2026 さらに {count} 件一致",
    installed: "インストール済み: {spec}",
    noServerFound:
      '{source} レジストリの {pages} ページを走査しましたが、"{target}" という名前のMCPサーバーは見つかりませんでした。',
    noServerTryMore: "試す: jupiter mcp install {target} --max-pages 100",
    noInstallMeta:
