...
export const de: TranslationSchema = {
  ...
  cli: {
    ...
    version: "Jupiter-Version ausgeben.",
    update: "Jupiters eigenen Update-Kanal prüfen.",
  },
  stats: {
    ...EN.stats,
    usageHint: "Führe `jupiter chat`, `jupiter code` oder `jupiter run <task>` aus – jeden Turn",
    usageDetail: "Hängt eine Zeile an das Log an; `jupiter stats` fasst sie zusammen.",
  },
  run: {
    ...EN.run,
    missingApiKey:
      "DEEPSEEK_API_KEY ist nicht gesetzt und stdin ist kein TTY (Nachfrage nicht möglich).\n" +
      "Setze die Umgebungsvariable oder starte einmal interaktiv `jupiter chat`, um einen Schlüssel zu speichern.\n",
  },
  sessions: {
    ...EN.sessions,
    emptyHint:
      "Noch keine gespeicherten Sitzungen – starte `jupiter chat` (Sitzungen werden automatisch gespeichert, außer mit --no-session).",
    listHeader: "Gespeicherte Sitzungen (~/.jupiter/sessions/):",
    inspectHint: "Ansehen:       jupiter sessions <name>",
    resumeHint: "Fortsetzen:    jupiter chat --session <name>",
  ...
  ui: {
    ...
    noConfigHint: "Ignoriere bei diesem Durchlauf die Datei ~/.jupiter/config.json",
    ...
    welcome: "Starte jederzeit `jupiter`, um zu chatten – deine Einstellungen bleiben gespeichert.",
  ...
  code: {
    ...EN.code,
    workspaceConflict:
      "⚠ Arbeitsbereich enthält Dateien einer anderen Agent-Plattform ({platforms}). Jupiter kann sie als Projektinhalt lesen; starte mit --dir <dein-projekt> neu, falls das nicht gewünscht ist.\n",
  ...
  slash: {
    ...
    memory: {
      ...
      description: "Pinned Memory anzeigen / verwalten (JUPITER.md + ~/.jupiter/memory)",
    },
    skill: {
      ...EN.slash.skill,
      description:
    ...
    hooks: {
      ...
      argsHint: "[Neu laden]",
      description:
        "Aktive Hooks auflisten (settings.json unter .jupiter/) · reload liest von Platte neu",
    },
    permissions: {
      ...
      description:
        "Shell-Allowlist anzeigen / bearbeiten (builtin schreibgeschützt · pro Projekt: ~/.jupiter/config.json)",
    },
    dashboard: {
      ...EN.slash.dashboard,
      argsHint: "[Stopp]",
    ...
    init: {
      ...EN.slash.init,
      description:
        "Projekt scannen und eine JUPITER.md-Baseline erstellen (Modell schreibt; mit /apply reviewen). `force` überschreibt vorhandene Datei.",
    },
    apply: {
      ...EN.slash.apply,
      description:
  ...
  wizard: {
    ...
    welcomeTitle: "Willkommen bei Jupiter.",
    ...
    mcpTitle: "Welche MCP-Server soll Jupiter für dich einrichten?",
  ...
  themePicker: {
    ...
    autoDesc: "JUPITER_THEME oder Standard verwenden",
  },
  planFlow: {
    ...EN.planFlow,
  ...
  errors: {
    ...EN.errors,
    contextOverflow:
      "Context-Überlauf (DeepSeek 400): Sitzungsverlauf ist {requested}, über dem Prompt-Limit des Modells (V4: 1M Tokens; legacy chat/reasoner: 131k). Meist ist ein einzelnes Tool-Ergebnis zu groß geworden. Jupiter begrenzt neue Tool-Ergebnisse auf 8k Tokens und heilt überdimensionierte Verläufe automatisch beim Sitzungsladen – ein Neustart behebt es oft. Falls es weiterhin überläuft, führe /new für einen frischen Start aus oder öffne /sessions und drücke [d], um diese Sitzung zu löschen.",
    ...
    concurrency429:
      "DeepSeek-Gleichzeitigkeitslimit erreicht (429): {inner}. Das Konto hat zu viele gleichzeitige Anfragen (Grenze: 500 für v4-pro, 2500 für v4-flash, summiert über alle API-Schlüssel des Kontos). Meist läuft ein weiterer Jupiter-Prozess mit demselben Schlüssel oder ein paralleler Subagent-Fan-out hat überzogen. Warte einige Sekunden und wiederhole, reduziere die Parallelität oder beantrage eine höhere Grenze unter https://platform.deepseek.com.",
    deepseek5xxHead:
      "DeepSeek-Dienst nicht verfügbar ({status}) — dies ist ein DeepSeek-seitiges Problem, nicht Jupiter. Bereits 4× mit Backoff wiederholt.",
  ...
  handlers: {
    ...
    basic: {
      ...
      newInfoSystemReloaded:
        " · JUPITER.md / Projekt-Memory neu geladen (nächster Turn zahlt einen Cache-Fehler)",
      helpTitle: "Befehle:",
      ...
      helpMemoryPin:
        "  #<notiz>                  <notiz> an <projekt>/JUPITER.md anhängen (commitierbar).",
      helpMemoryPinEx:
        "                             Beispiel: #findByEmail muss case-insensitive sein",
      helpMemoryGlobal:
        "  #g <notiz>                <notiz> an ~/.jupiter/JUPITER.md anhängen (global, niemals committed).",
      helpMemoryGlobalEx:
      ...
      helpSessionCustom: "  jupiter chat --session <name>   eine andere benannte Sitzung verwenden",
      helpSessionNone: "  jupiter chat --no-session       Persistenz für diesen Lauf deaktivieren",
      ...
      aboutHeader: "Jupiter v{version} — ein Cache-First-DeepSeek-Coding-Agent",
    ...
    admin: {
      ...
      hooksProjectFallback: "  · <projekt>/.jupiter/settings.json (Projekt)",
      ...
      updateCurrent: "Aktuell: Jupiter {version}",
      updateLatestPending: "Neueste:  (Jupiter-Update-Kanal ist nicht konfiguriert oder offline)",
      updateRetryHint:
        "Setze JUPITER_REGISTRY_URL oder JUPITER_NPM_PACKAGE, um Prüfungen zu aktivieren,",
      updateRetryHint2:
        "oder starte den Jupiter-Updater in einem anderen Terminal, sobald dein Kanal bereit ist.",
      updateLatest: "Neueste:  Jupiter {version}",
      updateUpToDate: "Du bist auf dem neuesten Stand. Nichts zu tun.",
      updateNpxHint:
        "Du verwendest npx — der nächste `npx jupiter ...`-Start lädt automatisch die neueste Version.",
      ...
      updateUpgradeCmd1: "  jupiter update            (interaktiv, --dry-run wird unterstützt)",
    ...
    edits: {
      ...EN.handlers.edits,
      undoCodeOnly:
        "/undo ist nur innerhalb von `jupiter code` verfügbar — der Chat-Modus wendet keine Edits an.",
      historyCodeOnly: "/history ist nur innerhalb von `jupiter code` verfügbar.",
      showCodeOnly: "/show ist nur innerhalb von `jupiter code` verfügbar.",
      applyCodeOnly:
        "/apply ist nur innerhalb von `jupiter code` verfügbar (hier gibt es nichts anzuwenden).",
      discardCodeOnly: "/discard ist nur innerhalb von `jupiter code` verfügbar.",
      planCodeOnly:
        "/plan ist nur innerhalb von `jupiter code` verfügbar — der Chat-Modus blockiert keine Tool-Schreibzugriffe.",
      planOn:
      ...
      modeCodeOnly: "/mode ist nur innerhalb von `jupiter code` verfügbar.",
      ...
      commitCodeOnly:
        "/commit ist nur innerhalb von `jupiter code` verfügbar (benötigt ein Git-Repo als Wurzel).",
      commitUsage:
        'Verwendung: /commit "deine Commit-Nachricht"  — führt `git add -A && git commit -m "…"` in {root} aus',
      walkCodeOnly: "/walk ist nur innerhalb von `jupiter code` verfügbar.",
      checkpointCodeOnly:
        "/checkpoint ist nur innerhalb von `jupiter code` verfügbar — der Chat-Modus wendet keine Edits an.",
      checkpointNone:
      ...
      restoreCodeOnly: "/restore ist nur innerhalb von `jupiter code` verfügbar.",
      ...
      cwdCodeOnly: "/cwd ist nur innerhalb von `jupiter code` verfügbar.",
      cwdUsage:
    ...
    model: {
      ...
      budgetNoCap:
        "Kein Sitzungsbudget festgelegt — Jupiter wird weiterlaufen, bis du es stoppst. Setze eines mit: /budget <usd>   (z.B. /budget 5)",
      budgetStatus:
    ...
    permissions: {
      ...
      mutateCodeOnly:
        "/permissions add / remove / clear sind nur innerhalb von `jupiter code` verfügbar — sie bearbeiten die projektbezogene Allowlist (`~/.jupiter/config.json` projects[<root>].shellAllowed).",
      addUsage:
    ...
    plans: {
      ...EN.handlers.plans,
      noSession:
        "Keine Sitzung angehängt — `/plans` ist pro Sitzung. Führe `jupiter code` in einem Projekt aus, um eine Sitzung zu erhalten.",
      activePlan:
      ...
      replayNoSession:
        "Keine Sitzung angehängt — `/replay` ist pro Sitzung. Führe `jupiter code` in einem Projekt aus, um eine Sitzung zu erhalten.",
      replayNoArchives:
    ...
    jobs: {
      ...EN.handlers.jobs,
      codeOnly: "/jobs ist nur innerhalb von `jupiter code` verfügbar.",
      killCodeOnly: "/kill ist nur innerhalb von `jupiter code` verfügbar.",
      logsCodeOnly: "/logs ist nur innerhalb von `jupiter code` verfügbar.",
      empty:
    ...
    memory: {
      ...EN.handlers.memory,
      disabled:
        "Memory ist deaktiviert (JUPITER_MEMORY=off in der Umgebung). Entferne die Variable zur Reaktivierung — es werden weder JUPITER.md noch ~/.jupiter/memory-Inhalte eingefügt.",
      noRoot:
        "Kein Arbeitsverzeichnis in dieser Sitzung — `/memory` benötigt ein Root, um JUPITER.md aufzulösen. (Läuft in einer Test-Umgebung?)",
      listEmpty:
        "Noch keine Benutzer-Memories. Das Modell kann `remember` aufrufen, um einen zu speichern, oder du kannst Dateien manuell in ~/.jupiter/memory/global/ oder dem projektspezifischen Unterverzeichnis erstellen.",
      ...
      layerGlobal: "  2. ~/.jupiter/memory/global/ — dein projektübergreifendes privates Memory.",
      layerProjectHash: "  3. ~/.jupiter/memory/<projekt-hash>/ — privates Memory dieses Projekts.",
    ...
    mcp: {
      ...EN.handlers.mcp,
      noServers:
        'Keine MCP-Server angehängt. Führe `jupiter setup` aus, um welche auszuwählen, oder starte mit --mcp "<spec>". `jupiter mcp list` zeigt den Katalog. Hinweis: vom Modell aufgerufene Shell-Befehle werden pro Aufruf abgefragt (einmal erlauben / immer erlauben / ablehnen) — kein globales Allow-All-Flag.',
      ...
      catalogHint:
        "Vollständiger Katalog: `jupiter mcp list` · tiefere Diagnose: `jupiter mcp inspect <spec>`.",
      fallbackServers: "MCP-Server ({count}):",
      fallbackTools: "Tools im Register ({count}):",
      fallbackChange: "Um diesen Satz zu ändern, beende und führe `jupiter setup` aus.",
      usageDisableEnable:
    ...
    skill: {
      ...EN.handlers.skill,
      listEmpty: "Keine Skills gefunden. Jupiter liest Skills von:",
      listProjectScope:
        "  · <projekt>/.jupiter/skills/<name>/SKILL.md  (oder <name>.md)  — Projekt-Bereich",
      listGlobalScope:
        "  · ~/.jupiter/skills/<name>/SKILL.md  (oder <name>.md)  — globaler Bereich",
      listProjectOnly: "  (Projekt-Bereich ist nur in `jupiter code` aktiv)",
  ...
  pathConfirm: {
    ...
    allowAlwaysDesc: "`{prefix}` für dieses Projekt merken (gespeichert in ~/.jupiter/config.json)",
    deny: "ablehnen",
  ...
  sessionPicker: {
    ...EN.sessionPicker,
    header: " ◈ JUPITER · Sitzung auswählen ",
  ...
  workspacePicker: {
    ...EN.workspacePicker,
    header: " ◈ JUPITER · Arbeitsbereich auswählen ",
  ...
  modelPicker: {
    ...EN.modelPicker,
    header: " ◈ JUPITER · Einrichtung auswählen ",
  ...
  startup: {
    ...EN.startup,
    codeRooted:
      '▸ jupiter code: verwurzelt in {rootDir}, Sitzung "{session}" · {tools} native Tool{s}{semantic}',
    ephemeral: "(ephemer)",
  ...
  webErrors: {
    ...
    tavilyMissingKey:
      "web_search: Tavily-Backend benötigt einen API-Schlüssel — setze TAVILY_API_KEY-Umgebungsvariable oder `tavilyApiKey` in ~/.jupiter/config.json; kostenlose 1000/Monat-Registrierung unter https://tavily.com",
    tavilyUnauthorized:
    ...
    perplexityMissingKey:
      "web_search: Perplexity-Backend benötigt einen API-Schlüssel — setze PERPLEXITY_API_KEY-Umgebungsvariable oder `perplexityApiKey` in ~/.jupiter/config.json; erhalte einen unter https://perplexity.ai/settings/api",
    ...
    exaMissingKey:
      "web_search: Exa-Backend benötigt einen API-Schlüssel — setze EXA_API_KEY-Umgebungsvariable oder `exaApiKey` in ~/.jupiter/config.json; kostenlose 1000/Monat-Registrierung unter https://exa.ai",
    exaUnauthorized:
    ...
    braveMissingKey:
      "web_search: Für Brave Search ist ein API-Schlüssel erforderlich — setze die Umgebungsvariable BRAVE_SEARCH_API_KEY (oder BRAVE_API_KEY) oder `braveApiKey` in ~/.jupiter/config.json; kostenlose Anmeldung mit 2000 Einheiten pro Monat unter https://brave.com/search/api/",
    braveUnauthorized:
    ...
    ollamaMissingKey:
      "Ollama benötigt einen API-Schlüssel — setze die Umgebungsvariable OLLAMA_API_KEY oder `ollamaApiKey` in ~/.jupiter/config.json; Schlüssel unter https://ollama.com/settings/keys",
    ollamaUnauthorized:
  ...
  mcpHealth: {
    ...
    emptyHint:
      "ℹ keine MCP-Server konfiguriert — versuche: `jupiter setup` zur erneuten Auswahl, oder `jupiter mcp install filesystem` · Shell-Befehle werden pro Aufruf abgefragt (einmal erlauben / immer erlauben / ablehnen), kein globales Allow-All",
  },
  denyContextInput: {
    ...EN.denyContextInput,
    description:
  ...
  mcpMarketplace: {
    ...
    bridgeReloadFailed:
      "✓ {name} installiert - starte `jupiter code` neu zur Verbindung (Neuladen fehlgeschlagen: {message})",
    restartBridge: "✓ {name} installiert - starte `jupiter code` neu zur Verbindung",
  ...
  mcpBrowser: {
    ...
    title: "◈ MCP-Browser",
    empty:
      "Keine MCP-Server angehängt. Führe `jupiter setup` aus, um welche auszuwählen, oder starte mit --mcp.",
  ...
  mcpLifecycle: {
    ...
    failedSetupHint:
      "→ führe `jupiter setup` aus, um diesen Eintrag zu entfernen, oder behebe das zugrunde liegende Problem (fehlendes npm-Paket, Netzwerk usw.).",
    failedSetupConfigHint:
      "→ führe `jupiter setup` aus, um fehlerhafte Einträge aus deiner gespeicherten Konfiguration zu entfernen.",
    abortedHint:
  ...
  checkpointPicker: {
    ...
    header: " ◈ JUPITER · Checkpoint auswählen ",
  ...
  mcpCli: {
    ...
    morePagesAvailable: "▸ mehr Seiten verfügbar — `jupiter mcp list --pages <n>` oder --all",
    installHint: "Installieren:  jupiter mcp install <name>",
    usageSearch: "Verwendung: jupiter mcp search <abfrage>",
    usageInstall: "Verwendung: jupiter mcp install <name>",
    ...
    moreLoaded: "… {count} weitere geladen — verwende `jupiter mcp search <abfrage>` zum Filtern",
    moreMatches: "… {count} weitere Treffer",
    installed: "Installiert: {spec}",
    noServerFound:
      'Kein MCP-Server namens "{target}" gefunden nach {pages} Seite(n) der {source}-Registry.',
    noServerTryMore: "Versuche: jupiter mcp install {target} --max-pages 100",
    noInstallMeta:
