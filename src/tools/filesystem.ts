...
// list, minus `.jupiter` — the indexer shouldn't embed session logs / cache, but
// user skills live at `<root>/.jupiter/skills/` (and `~/.jupiter/skills/`) and
// must stay reachable to read_file / search_files / search_content (#1357).
const SKIP_DIR_NAMES: ReadonlySet<string> = new Set(
  DEFAULT_INDEX_EXCLUDES.dirs.filter((d) => d !== ".jupiter"),
);
...
export function registerFilesystemTools(
  ...
  /** Subdir JUPITER.md paths already injected this session (#1033). Reset per toolset, so each tab/session re-injects on first relevant read. */
  const shownSubdirMemory = new Set<string>();

  /** Prepend any not-yet-shown ancestor JUPITER.md (between absPath's dir and rootDir) to a read_file body. Outer dirs first so broad rules read before specific overrides. */
  ...
  /** Same idea as withSubdirMemory but for list_directory — includes the listed dir's own JUPITER.md, not just ancestors. */
