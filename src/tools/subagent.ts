...
import { loadSessionMeta, patchSessionMeta, timestampSuffix } from "../memory/session.js";
import {
  NEGATIVE_CLAIM_RULE,
...
export interface SpawnSubagentOptions {
  ...
  /** Parent workspace root, persisted onto the child transcript so desktop grouping stays stable. */
  projectRoot?: string;
}
...
const SUBAGENT_BASE_SYSTEM = `You are a Jupiter subagent. The parent agent spawned you to handle one focused subtask, then return.
...
export async function spawnSubagent(opts: SpawnSubagentOptions): Promise<SubagentResult> {
  ...
  if (opts.projectRoot) {
    try {
      ...
      if (typeof meta.workspace !== "string" || meta.workspace.trim().length === 0) {
        patchSessionMeta(sessionName, { workspace: opts.projectRoot });
      }
    } catch {
      // Workspace metadata is for desktop filtering only; don't block the child loop.
    }
  }
...
/** Library surface only — `jupiter code` uses Skills `runAs: subagent` as the user-facing path. */
export function registerSubagentTool(
  ...
  // spawn would (a) make the child prefix unstable when JUPITER.md
