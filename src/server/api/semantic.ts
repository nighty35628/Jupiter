/** Job state in a module-scoped Map keyed by project root so multi-root dashboards don't collide; CLI `jupiter index` runs independently. */
  ...
  if (!root) {
    return {
      ...
      body: {
        attached: false,
        reason:
          "Semantic indexing requires a code-mode session — run `/dashboard` from inside `jupiter code` instead of standalone `jupiter dashboard`.",
      },
    };
  }
