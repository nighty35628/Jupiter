...
function parseTranscript(path: string, maxBytes = 4 * 1024 * 1024): SessionMessage[] {
  ...
  for (const line of raw.split(/\r?\n/)) {
    ...
    try {
      ...
      /* skip malformed line — same rule as the rest of Jupiter's JSONL readers */
    }
  }
  return out;
}
...
export async function handleSessions(
  ...
  // sidebar; users have reported 10 000+ entries in `~/.jupiter/sessions/`.
  if (method === "GET" && rest.length === 0) {
    const workspaceFilter = ctx.getCurrentCwd?.();
    const sessions = (
      workspaceFilter ? listSessionsForWorkspace(workspaceFilter) : listSessions()
