...
export async function handleHealth(
  ...
  const home = homedir();
  const jupiterHome = join(home, ".jupiter");

  const sessionsStat = dirSize(join(jupiterHome, "sessions"));
  const memoryStat = dirSize(join(jupiterHome, "memory"));
  const semanticStat = dirSize(join(jupiterHome, "semantic"));
