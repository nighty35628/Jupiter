...
export function checkAuth(
  ...
  const headerToken =
    typeof req.headers["x-jupiter-token"] === "string"
      ? (req.headers["x-jupiter-token"] as string)
      : "";
  ...
  if (isMutation) {
    ...
    if (!headerToken || !constantTimeEquals(headerToken, expectedToken)) {
      return {
        ...
        body: JSON.stringify({
          error:
            "mutation requires X-Jupiter-Token header (CSRF defence — query token alone is rejected for POST/DELETE).",
        }),
      };
    }
    return null;
