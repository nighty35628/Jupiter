...
client, hand-rolled in TypeScript. Lets Jupiter consume tools from any
...
- **Zero runtime deps** for this module. Consistent with Jupiter's
  policy of owning the wire format where it matters.
- **Surface tuning**: we only implement what Jupiter actually uses —
...
| feature | status | note |
|---|---|---|
| CLI wiring (`jupiter chat --mcp <cmd>`) | ✅ shipped | see Usage below |
...
| Resources / `resources/list` / `resources/read` | deferred | Jupiter doesn't surface resources today |
...
| MCP server that Jupiter exposes | never | out of scope — Jupiter is a client |
...
jupiter chat --mcp "node --import tsx examples/mcp-server-demo.ts"

# Official filesystem server:
jupiter chat --mcp "npx -y @modelcontextprotocol/server-filesystem /tmp/safe-dir"
...
jupiter run "list files in /tmp/safe-dir" \
...
`ImmutablePrefix` for the model, and every call goes through Jupiter's
...
// 2. Bridge its tools into a Jupiter ToolRegistry
...
The payoff: the filesystem server's tools now inherit Jupiter's
...
- **Debugging dropped lines**: set `JUPITER_DEBUG_MCP=1` to print each
