...
export function resolveDefaults(flags: RawCliFlags): ResolvedDefaults {
  const cfg: JupiterConfig = flags.noConfig ? {} : readConfig();
  const model =
...
function mergeDotMcpJson(cfg: JupiterConfig, projectRoot: string): JupiterConfig {
...
export function resolveBareCommandMode(
  cfg: Pick<JupiterConfig, "setupCompleted">,
): "setup" | "code" {
