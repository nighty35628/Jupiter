...
function checkProxy(): Check[] {
    ...
    return [
      {
        ...
        label: "http proxy   ",
        level: "ok",
        detail: `${urlSource}=${redacted} is set but cfg.proxy.disabled — Jupiter routes direct`,
      },
    ];
  }
  ...
  const sourceSummary = [
    ...
    resolved.envJupiter.length > 0 ? `JUPITER ${resolved.envJupiter.length}` : null,
...
async function checkApiKey(): Promise<Check> {
  ...
  return {
    ...
    level: "fail",
    detail:
      "not set — `jupiter setup` to save one, or export DEEPSEEK_API_KEY. Get a key at https://platform.deepseek.com/api_keys",
  };
}
...
async function checkConfig(): Promise<Check> {
  ...
  if (!existsSync(path)) {
    return {
      ...
      label: "config       ",
      level: "warn",
      detail: "missing — running with library defaults. `jupiter setup` writes one.",
    };
  }
  try {
  ...
  try {
    ...
    if (stale > 0) {
      return {
        ...
        label: "sessions     ",
        level: "warn",
        detail: `${detail} · ${stale} idle ≥90d (run \`jupiter prune-sessions\`)`,
      };
    }
...
async function checkOllama(projectRoot: string): Promise<Check> {
  ...
  if (!exists) {
    return {
      ...
      label: "semantic     ",
      level: "ok",
      detail: "not in use (no semantic index built; `jupiter index` to enable)",
    };
  }
  ...
  try {
    const model = meta?.model || process.env.JUPITER_EMBED_MODEL || "nomic-embed-text";
...
function readSemanticMeta(
  ...
  try {
    const raw = readFileSync(join(projectRoot, ".jupiter", "semantic", "index.meta.json"), "utf8");
...
async function checkProject(projectRoot: string): Promise<Check> {
  // Heuristic: a "real" project has either .git, JUPITER.md, or
  // package.json. Lacking all three, `jupiter code` still works but
  // @-mentions and the project-memory pin won't surface much.
  const markers = [".git", "JUPITER.md", "package.json", "pyproject.toml", "Cargo.toml", "go.mod"];
  ...
  if (found.length === 0) {
    return {
      ...
      label: "project      ",
      level: "warn",
      detail: `${projectRoot} has none of: ${markers.slice(0, 3).join(", ")} … — \`jupiter code\` will still run, but @-mentions and project memory have nothing to anchor`,
    };
  }
  return {
    id: "project",
...
export async function doctorCommand(opts: DoctorOptions = {}): Promise<void> {
  ...
  if (!json) {
    console.log(`${color(`jupiter ${VERSION}  ·  doctor`, "1")}  (cwd: ${projectRoot})`);
