/** First-run / re-configure wizard — saves to `~/.jupiter/config.json`. */
...
export function Wizard({
  ...
  const [previewTheme, setPreviewTheme] = useState<ThemeName>(() =>
    resolveThemePreference(initial?.theme ?? loadTheme(), process.env.JUPITER_THEME),
  );

  const [step, setStep] = useState<Step>("language");
  const [data, setData] = useState<WizardData>(() => ({
    language: getLanguage(),
    theme: resolveThemePreference(initial?.theme ?? loadTheme(), process.env.JUPITER_THEME),
...
function placeholderFor(entry: CatalogEntry): string {
  if (entry.name === "filesystem") return "e.g. /tmp/jupiter-sandbox";
 ...
 * `mcpCommandFor` produces for `jupiter mcp list`, minus the leading
