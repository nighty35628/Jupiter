...
import { derivePrefix, toApprovalPrompt } from "@jupiter/core-utils";
...
export interface AppProps {
  ...
  system: string;
  /** Re-runs the prompt builder on /new so JUPITER.md edits don't need a restart. Must produce the same shape as `system` was built from. */
   ...
   * apply them to disk under `rootDir`. Set by `jupiter code`. The
...
export function App(props: AppProps): React.ReactElement {
  ...
  const [themeName, setThemeName] = React.useState<ThemeName>(() =>
    resolveThemePreference(loadTheme(), process.env.JUPITER_THEME),
  );
...
function AppInner({
  ...
  // typed Event log accumulates at `~/.jupiter/sessions/<name>.events.jsonl`.
  ...
  const startWalkthrough = useCallback((): string => {
    if (!codeMode) {
      return "/walk is only available inside `jupiter code`.";
    }
  ...
  const handleQQThemePick = useCallback(
    (target: ThemeChoice): string => {
      saveTheme(target);
      const active = resolveThemePreference(target, process.env.JUPITER_THEME);
      setThemeName(active);
  ...
  const dingtalk = useDingTalkChannel({
    codeMode: !!codeMode,
  ...
  const handleSubmit = useCallback(
    async (raw: string) => {
      ...
      const dingtalkIncoming =
        qqIncoming?.handled || qqIncoming?.fromQQ || feishuIncoming?.fromFeishu
          ? null
          : dingtalk.parseSubmit(raw);
      const incoming =
        qqIncoming?.handled || qqIncoming?.fromQQ
          ? qqIncoming
          : feishuIncoming?.fromFeishu
            ? feishuIncoming
            : dingtalkIncoming?.fromDingTalk
              ? dingtalkIncoming
      ...
      const fromDingTalk = "fromDingTalk" in incoming && incoming.fromDingTalk;
      ...
      // a JUPITER.md so future sessions pin the note in the immutable
      ...
      dingtalk.noteTurnFromDingTalk(fromDingTalk);
      ...
      try {
        ...
        dingtalk.maybeSendFinalReply(lastAssistantText);
              ...
              {stagedInput ? (
                  ...
                  onChoose={(outcome) => {
                    if (outcome.kind === "open") {
                      ...
                      if (onSwitchSession) {
                        ...
                        log.pushInfo(
                          `to switch to "${outcome.name}", quit and run: jupiter chat --session ${outcome.name}`,
                        );
                      }
                      return;
                    }
                    if (outcome.kind === "new") {
                      ...
                      if (onSwitchSession) {
                        ...
                        log.pushInfo(
                          "to start a fresh session, quit and run: jupiter chat (no --session flag)",
                        );
                      }
                      return;
                    }
                ...
                <ThemePicker
                  ...
                  onChoose={(outcome) => {
                    ...
                    const active = resolveThemePreference(outcome.value, process.env.JUPITER_THEME);
                    setThemeName(active);
