...
const INIT_PROMPT = [
  "# Task: Initialize JUPITER.md",
  "",
  "I want you to generate a JUPITER.md at the project root that captures",
  "the working knowledge a future Jupiter session needs to be productive",
  ...
  "5. Write JUPITER.md with the sections below, skipping any you can't",
  "   fill from evidence.",
  "",
  ...
  "## Output",
  "",
  "Write the result to `JUPITER.md` in the project root using the",
  "filesystem tools (edit_file with empty SEARCH if creating new,",
  "write_file if overwriting). After writing, report the result in one",
  "short final message: whether `JUPITER.md` was created or updated as",
  "a pending edit, which sections you included, and the next step:",
  "review with /walk or apply with /apply. If you were blocked, report",
  "what failed and what you already checked. Do not propose unrelated",
  "follow-up tasks.",
  "",
  "Start now.",
].join("\n");
...
const init: SlashHandler = (args, _loop, ctx) => {
  ...
  const target = pathMod.join(ctx.codeRoot, "JUPITER.md");
