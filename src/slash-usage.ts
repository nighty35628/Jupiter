...
export function slashUsagePath(): string {
  const override = process.env.JUPITER_SLASH_USAGE_PATH;
  if (override) return override;
  return join(homedir(), ".jupiter", "slash-usage.json");
}
...
/** Read-modify-write so two concurrent jupiter processes don't clobber each other's counts. */
