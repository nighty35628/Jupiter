/** VERSION sourced from package.json; latest-check returns null on any failure. */
...
const JUPITER_REGISTRY_URL_ENV = "JUPITER_REGISTRY_URL";
const JUPITER_NPM_PACKAGE_ENV = "JUPITER_NPM_PACKAGE";

/** TTL for the on-disk cache entry. 24h keeps noise low; explicit update checks
 * pass `force: true`. */
...
/** `name === "jupiter"` guard avoids picking up an outer package.json when loaded as a dep. */
  ...
  try {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (existsSync(p)) {
        ...
        if (pkg?.name === "jupiter" && typeof pkg.version === "string") {
          return pkg.version;
        }
      }
...
function cachePath(homeDirOverride?: string): string {
  return join(homeDirOverride ?? homedir(), ".jupiter", "version-cache.json");
}
...
export interface GetLatestVersionOptions {
  /** Ignore the cached entry and always fetch fresh. Used by `jupiter update`. */
  force?: boolean;
...
export function configuredLatestRegistryUrl(
  env: Record<string, string | undefined> = process.env,
): string | null {
  const explicit = env[JUPITER_REGISTRY_URL_ENV]?.trim();
  if (explicit) return explicit;
  const packageName = env[JUPITER_NPM_PACKAGE_ENV]?.trim();
...
export async function getLatestVersion(opts: GetLatestVersionOptions = {}): Promise<string | null> {
  ...
  const url = opts.registryUrl ?? configuredLatestRegistryUrl();
  if (!url) return null;
...
export function detectInstallSource(bin?: string): InstallSource {
  ...
  if (/\/node_modules\/jupiter(\b|\/)/.test(norm)) return "npm";
  return "unknown";
}
...
export function detectNpmInstallPrefix(bin?: string): string | null {
  ...
  const posix = norm.match(/^(.+?)\/lib\/node_modules\/jupiter(?:\/|$)/i);
  if (posix) return posix[1] ?? null;
  const win = norm.match(/^(.+?)\/node_modules\/jupiter(?:\/|$)/i);
