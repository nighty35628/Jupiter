...
// `jupiter doctor` reachability checks; non-negotiable.
...
// `JUPITER_PROXY_DEEPSEEK_DIRECT=0` (env) for corporate firewalls that block
...
export interface InstallProxyOptions {
  ...
  url?: string;
  /** Additional NO_PROXY patterns layered on top of defaults + env. Sourced from `cfg.proxy.noProxy` / `JUPITER_NO_PROXY`. */
...
export interface ResolvedNoProxy {
  ...
  envJupiter: NoProxyPattern[];
  extra: NoProxyPattern[];
  /** Defaults + env + JUPITER + extra concatenated. The same list `installProxyIfConfigured` uses. */
  all: NoProxyPattern[];
}

/** Env `JUPITER_PROXY_DEEPSEEK_DIRECT` (1/0/true/false/yes/no/on/off) overrides config when set; defaults true. Per issue #1497, corporate firewalls need the env knob since they often can't edit `~/.jupiter/config.json` ergonomically. */
export function resolveBypassDeepSeekDirect(
  env: NodeJS.ProcessEnv,
  configValue: boolean | undefined,
): boolean {
  const raw = env.JUPITER_PROXY_DEEPSEEK_DIRECT;
...
/** Merge default + env + JUPITER_NO_PROXY + opts.extraNoProxy into one resolved view. Same composition as installProxyIfConfigured so /doctor can show what's actually applied. */
export function resolveNoProxy(
  ...
  const envJupiter = parseNoProxy(
    typeof env.JUPITER_NO_PROXY === "string" ? env.JUPITER_NO_PROXY : null,
  );
  ...
  return {
    ...
    envJupiter,
    extra,
    all: [...defaults, ...envSystem, ...envJupiter, ...extra],
  };
}
...
export function installProxyIfConfigured(
  ...
  // Default whitelist always applies; env NO_PROXY, JUPITER_NO_PROXY, and
