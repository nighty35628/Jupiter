...
export function resolveDataPath(): string {
  if (process.env.JUPITER_TOKENIZER_PATH) return process.env.JUPITER_TOKENIZER_PATH;
  ...
  try {
    ...
    candidates.push(
      join(dirname(req.resolve("jupiter/package.json")), "data", "deepseek-tokenizer.json.gz"),
    );
  } catch {
    /* Not installed as `jupiter/` — the earlier candidates still may hit. */
  }
