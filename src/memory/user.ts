/** User-private memory pinned into the immutable prefix; distinct from committable JUPITER.md. */
...
export interface MemoryStoreOptions {
  /** Override `~/.jupiter` — tests set this to a tmpdir. */
  homeDir?: string;
...
export class MemoryStore {
  ...
  constructor(opts: MemoryStoreOptions = {}) {
    this.homeDir = opts.homeDir ?? join(homedir(), ".jupiter");
...
export function readGlobalJupiterMemory(
  homeDir: string = join(homedir(), ".jupiter"),
): { path: string; content: string; originalChars: number; truncated: boolean } | null {
  const path = join(homeDir, "JUPITER.md");
...
export function applyGlobalJupiterMemory(basePrompt: string, homeDir?: string): string {
  ...
  return [
    basePrompt,
    "",
    "# Global memory (~/.jupiter/JUPITER.md)",
    "",
 ...
 *  Same cap as global Jupiter memory (8000 chars). */
...
function highPriorityBlock(entries: MemoryEntry[], cfg?: JupiterConfig): string | null {
...
export function applyUserMemory(
  basePrompt: string,
  opts: { homeDir?: string; projectRoot?: string; cfg?: JupiterConfig } = {},
): string {
  if (!memoryEnabled()) return basePrompt;
  ...
  if (global) {
    parts.push(
      "",
      "# User memory — global (~/.jupiter/memory/global/MEMORY.md)",
      "",
...
export function applyMemoryStack(
  basePrompt: string,
  rootDir: string,
  opts: { homeDir?: string; cfg?: JupiterConfig } = {},
): string {
