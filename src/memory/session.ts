/** JSONL append-only message log under `~/.jupiter/sessions/`; concurrent-write safe. */
...
export function sessionsDir(): string {
  return join(homedir(), ".jupiter", "sessions");
}
...
export function isInternalSessionName(name: string): boolean {
  return sanitizeName(name).startsWith("subagent-");
}
