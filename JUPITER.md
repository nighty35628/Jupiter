...
- **This IS Jupiter** — edits to `src/loop.ts`, `src/repair/`, `src/tools/`, `src/mcp/` affect every session. Test before publishing.
...
- **`.jupiter/semantic/`** is auto-generated vector index. Never hand-edit.
- **`sessions/` and `.jupiter/sessions/`** are user-private, git-ignored (per `.gitignore`).
