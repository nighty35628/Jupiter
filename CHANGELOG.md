...
other local AI apps can be imported into Jupiter (#1894).
...
`JUPITER.md` (#1694) lets Claude Code users carry their project /
user memory directly into Jupiter without rewriting it.
...
- `JUPITER_ACP_SYSTEM_APPEND` env var for system-prompt injection (#1737)
...
- Per-component render trace gated by `JUPITER_TRACE_RENDERS` (#1747);
...
jupiter@0.50.0 code` failed during install with a `dashboard/package.json
...
Ink-native instead of fighting it. Behind `JUPITER_STATIC_HISTORY` at
...
- `--no-proxy` / `JUPITER_NO_PROXY` opt-out is honored for the
...
`jupiter` (#1440), so `npx dsnix` is now equivalent to `npx jupiter`
...
(#1387), giving Jupiter structural code navigation instead of plain
...
no longer ping-pong (#1392), and FS tools walk `.jupiter/` so user
...
**Hotfix — `npx jupiter@latest` install.** `0.47.1` shipped with
`@jupiter/core-utils` listed under runtime `dependencies` as
...
**Other:**
- `feat(core-utils)` new `@jupiter/core-utils` workspace package —
...
**Claude-ecosystem compat.** Jupiter now reads `.mcp.json` and
...
**Breaking — Rust renderer removed.** Jupiter is back to a pure Ink/Node
TUI. The `jupiter-render` ratatui crate, the five
`@jupiter/render-{platform}-{arch}` optional sub-packages, the NAPI
...
Notes for users:
- `JUPITER_RENDERER`, `JUPITER_RENDER_BIN`, `JUPITER_RENDER_CMD`,
  `JUPITER_INPUT_CMD`, `JUPITER_RENDERER_INTEGRATED` env vars are no-ops
...
- `--no-alt-screen` / `--no-mouse` flags also gone — Ink defaults take
  over end-to-end.
- `npm install jupiter@0.46.0` no longer pulls a per-platform binary —
...
**Fix:** macOS hang on `npx jupiter@next code` — keep-alive interval
...
- `trace.ts` now logs a clear warning to stderr when `resolveRenderer`
  ...
  `~/.jupiter/rust-render-stderr.log`.

## [0.44.2-rc.1] — 2026-05-17

**Fix:** macOS `npx jupiter code` (default rust + integrated TUI) exited
...
**Fix:** Mac/Linux `npx jupiter@latest` failed with `EACCES` when spawning
the rust renderer (`spawn ... jupiter-render EACCES`). `actions/download-artifact@v4`
...
- Each `@jupiter/render-*` subpackage now declares its binary in the
...
**Headline:** The Rust TUI is now the default TUI. `npx jupiter@latest`
...
Bare `jupiter` (no subcommand) routes to `code` in the cwd instead of
chat — explicit `jupiter chat` still works.

**Note:** Existing users with `JUPITER_RENDERER=rust` or
`JUPITER_RENDERER_INTEGRATED=1` set in their shell rc will see no
...
- feat(rust): rust TUI is the default. `--node` flag (or
  `JUPITER_RENDERER=node`) opts back to the Ink/Node renderer. Auto-
...
- feat(rust): integrated mode is the default once the rust renderer
  is active. `JUPITER_RENDERER_INTEGRATED=0` opts back to the
  --emit-input split keystroke bus (debug only) (#1081)
- feat(cli): bare `jupiter` (no subcommand) now launches `code` in
  the cwd. Drops the project-marker heuristic that used to route to
  `chat` outside of project dirs; `jupiter chat` stays explicit (#1081)
- feat(scene): renderer-resolver picks the binary by priority chain —
  `JUPITER_RENDER_CMD` / `JUPITER_INPUT_CMD` env (full-command
  override) > `JUPITER_RENDER_BIN` env (single binary path) >
  `@jupiter/render-{platform}-{arch}` optional dependency > source-
  tree `target/release/jupiter-render(.exe)` > `target/debug/...` >
  `cargo run --bin jupiter-render` (source tree + cargo on PATH) >
...
- feat(release): per-platform `@jupiter/render-*` subpackages
...
- fix(code): honor the configured preset instead of hardcoding flash.
  (#824)
- fix(cli): route bare project launches (`jupiter` with no
...
- fix(dashboard): swallow third-party-origin errors in the global
  overlay — extension scripts no longer surface as Jupiter crashes.
  (#825)
...
`--mcp-prefix`) make `jupiter acp` ready for non-interactive callers
...
field-reported fixes for `/new` silently keeping the old JUPITER.md,
...
- feat(acp): three-stage rollout of `jupiter acp`, the headless ACP
...
- feat(acp): `--yolo` flag bypasses plan checkpoints without mutating
  `~/.jupiter/config.json` — process-per-task callers don't have to
...
- fix(loop): `/new` now re-runs the system-prompt builder, so editing
  JUPITER.md and starting a fresh conversation actually picks up the
...
client (`jupiter desktop`) wired against the agent loop with multi-
...
window), `jupiter code` no longer boots dead when a user has only
configured their key via `jupiter setup`, and a half-dozen MCP /
...
- fix(tui): CardStream window position is now quantized to
  ...
  pattern). (#700, #702)
- fix(code): `jupiter code` now calls `loadDotenv()` + bridges
  `~/.jupiter/config.json` → `process.env.DEEPSEEK_API_KEY` like
...
- fix(skills, memory): frontmatter parser handles BOM, folded lines,
  and quoted values; previously a BOM-prefixed `JUPITER.md` was
...
when jupiter is installed as a dependency. The install-time
...
separate `JUPITER.md`, and `HTTPS_PROXY` / `HTTP_PROXY` / `ALL_PROXY`
...
- feat(memory): `readProjectMemory` walks `JUPITER.md` → `AGENTS.md` →
  ...
  jupiter-only memory file. (#635, #636)
...
- fix(loop): `/new` truncated `~/.jupiter/sessions/code-<project>.jsonl`
...
- fix(ui): the default frame flush was 16ms (60Hz), which on
  ...
  `JUPITER_UI=plain` escape hatch (which suppressed every live row)
  is removed since the new default addresses the same terminals
  without losing the spinner / status line / live cards. Override
  via `JUPITER_FLUSH_MS=16` for terminals with atomic frame swap.
  (#570)

**Features:**

- feat(ui): boot splash for `jupiter code` / `jupiter chat`. Cold
  ...
  JUPITER wordmark lands cleanly and AppInner's heavy first-paint
...
- fix(prompt): when the workspace root contained another agent
  ...
  `JUPITER.md`) the model would browse those files and claim a
  ...
  what Jupiter is; identity questions are answered from the prompt,
...
`jupiter update` (no more forced `npm install -g` for bun/pnpm
...
- feat(update): `jupiter update` respects the install source
...
- perf(tui): streaming flush rate tuned to 60Hz default. Earlier
  ...
  `JUPITER_FLUSH_MS` overrides for hosts that need it. (#515, #517)
...
- feat(tools): `create_skill` + `add_mcp_server` — let the model
  ...
  alongside native filesystem / shell tools in `jupiter code`.
  (#498, closes #494)
...
- fix(doctor): tokenizer check now finds the file. The runtime
  ...
  `createRequire("jupiter/package.json")` probe and worked
...
profiler gated behind `JUPITER_PROFILE_STARTUP=1`. Stage 2 lazy-
imports every per-command module and the dashboard server, paying
for the chat UI only when `jupiter code` actually runs. `jupiter
version` and `jupiter --help` drop ~290ms (~440ms → ~140ms);
`jupiter code` is unchanged on the hot path. Critical bug fix at
...
- perf(cli): `JUPITER_PROFILE_STARTUP=1` cold-start profiler. Marks
...
- perf(cli): lazy-import every per-command module. Each
  `jupiter <subcommand>` only loads its own command's chunk. tsup
  splits, Node loads on first invocation. `jupiter version` and
  `jupiter --help` drop ~290ms (~440ms → ~140ms); `jupiter code`
...
search` footer. A new `~/.jupiter/slash-usage.json` counter
...
- feat(slash): grouped suggestions + usage telemetry.
  ...
  behind a footer. New `~/.jupiter/slash-usage.json` counter
...
**Headline:** a Mac user reported a DeepSeek 503 day where Jupiter
...
- feat(loop): friendly DeepSeek 5xx error with reachability probe.
  ...
  a DeepSeek-side problem, not Jupiter" and link
...
- feat(wizard): first-launch language picker + full i18n. New
  ...
  language. Re-running `jupiter setup` opens at the same step with
...
- feat(skills): `/skill new <name>` scaffolds a stub at
  `<project>/.jupiter/skills/<name>.md` with minimal frontmatter
  + a comment block listing the optional knobs (`runAs`,
  `allowed-tools`, `model`). `/skill new <name> --global` writes
  under `~/.jupiter/skills/` for cross-project use; auto-falls-
...
- fix(tty): drain pending feature-detection replies on exit. Linux
  ...
  fish / bash after exiting jupiter — those bytes are responses to
...
- feat(ui): surface `--dir` / pinned workspace for first-time users.
  ...
  filesystem sandbox-escape error points at `jupiter code --dir
...
18.3 → 19.2, the cell-diff renderer is retired, and `jupiter code` /
`jupiter chat` default to alt-screen with row-precision virtual
...
- feat(web): configurable `web_search` backend with SearXNG support.
  ...
  persisted to `~/.jupiter/config.json`. Mojeek remains the default;
...
- feat(loop): chunked parallel tool dispatch. Replaces `for...of +
  ...
  declared order. Two new env knobs: `JUPITER_PARALLEL_MAX` (chunk
  size cap, default 3, hard max 16) and `JUPITER_TOOL_DISPATCH=serial`
...
- feat(tui): right-side context panel showing the active plan
  ...
  persists in `~/.jupiter/config.json.sidebarOpen`. Refuses below 88
...
- feat(prompt): full readline shortcut set wired into the prompt
  ...
  `Ctrl+U` keeps Jupiter's "clear whole buffer" behaviour, not
...
- feat(mcp): new `/mcp reconnect <name>` slash subcommand. Re-handshakes
  ...
  reorder / remove drift is refused with a clear "restart Jupiter to
...
- feat(mcp): `/mcp disable <name>` and `/mcp enable <name>` slash
  subcommands persist a `mcpDisabled` list to `~/.jupiter/config.json`.
...
- `engines.node` bumped from `>=20.10` to `>=22`. Node 20 reached
  end-of-life on 2026-04-30; `npm install jupiter` on Node 20 will now
  print an `EBADENGINE` warning. Tested CI surface trimmed to a single
  Node 22 job. (#98)

**Fixes:**

- fix(code): `jupiter code` now bridges MCP servers from
  `~/.jupiter/config.json`, matching `jupiter chat` behaviour.
...
- docs(readme): designer pass — drop redundant `# Jupiter` H1 (the
...
**Headline:** `jupiter index` is now config-driven — what gets walked
is defined entirely by `~/.jupiter/config.json` (with sensible
...
- feat(index): new `index` block in `JupiterConfig` (`excludeDirs`,
...
- feat(index): glob excludes via `picomatch` syntax in
  ...
  supported).
- feat(cli): `jupiter index` success line now prints a per-reason
...
API"). Jupiter's whitelist-by-model in `assistantMessage()` was too
...
on v0.16.0: 24/24 jupiter runs were failing.
...
- feat(ui): app-owned mouse selection. Plain drag paints a reverse-
  ...
  clipboard) plus a `<tmpdir>/jupiter-clip-<ts>.txt` fallback for
...
- feat(cli): `jupiter events <name>` — inspect any session's event
  stream from the command line. Filters by event variant
  (`jupiter events ToolCallStarted`), tail mode, JSON output for
...
Previously you had to exit the TUI, run `jupiter index`, wait,
...
- Standalone `jupiter dashboard` mode shows a polite "code-mode
...
- Auto-launch the embedded dashboard when `jupiter code` /
  `jupiter chat` mount. Failures are silent (a missing dashboard
...
- Memory — JUPITER.md + global / project private memory editor
...
- `cacheSavingsUsd(model, hitTokens)` in `src/telemetry.ts` — USD
  ...
  for cached tokens). Surfaced in `jupiter stats` dashboard +
...
- **Spec parser** — `streamable+http(s)://` prefix routes to the
  ...
  Wired through `chat.tsx`, `run.ts`, and `jupiter mcp inspect`.
...
**Headline:** `/init` synthesizes a baseline JUPITER.md so a new
project starts with context instead of cold. Closes the gap with
Claude Code's `/init`, scoped to the structure JUPITER expects.

### Added

- **`/init`** — model-driven JUPITER.md generator. The slash
  ...
  capped at 80 lines / 3KB so JUPITER.md doesn't bloat the system
  ...
  overwrite an existing JUPITER.md without `/init force`. Removes
...
backed embedding index, `jupiter index` CLI with progress spinner, a
...
- **`jupiter index`** — new CLI command that walks the project, line-
  windows source files, embeds via Ollama (`nomic-embed-text` by
  default, ~274 MB once), and persists a JSONL index at
  `.jupiter/semantic/`. Incremental by default (mtime-based), with
...
- **`semantic_search` tool** — registered in `jupiter code` only when
...
- **Bilingual UI** — `src/index/semantic/i18n.ts` with EN/ZH dicts
  ...
  detection: `JUPITER_LANG` override → `LANG`/`LC_ALL`/`LC_MESSAGES`
...
- **Default model is now `deepseek-v4-flash`**, not `deepseek-v4-pro`.
  `jupiter code`, `jupiter chat`, and subagents all land on flash
...
**Headline:** `jupiter code` gets a proper review gate, background
...
- **Onboarding tip** — first `jupiter code` launch after upgrade
...
- **`reasoningEffort` persistence** — `/effort high` now writes the
  choice to `~/.jupiter/config.json` and the loop picks it up at
...
**Headline:** `jupiter stats` is now a cross-session cost dashboard.

Every turn `jupiter chat|code|run` executes now appends one line to
`~/.jupiter/usage.jsonl` carrying tokens + cost + the equivalent
Claude Sonnet 4.6 cost. `jupiter stats` (no arg) rolls that log up
into today / week / month / all-time windows:

```
Jupiter usage — /Users/you/.jupiter/usage.jsonl (2.3 KB)
...
Back-compat: `jupiter stats <transcript>` still works — passing a
...
- **`/stats` slash** — same dashboard, in-session. Reads
  `~/.jupiter/usage.jsonl` and renders via the shared
...
- **Wire-up** in `src/cli/ui/App.tsx` (assistant_final event) and
  ...
  log as TUI turns).
- **Upgraded `jupiter stats`**. No-arg → dashboard; transcript arg
...
A hook is a shell command. Jupiter invokes it with stdin = a JSON
...
```json
// <project>/.jupiter/settings.json   ← committable
// ~/.jupiter/settings.json           ← per-user
{
  "hooks": {
    "PreToolUse":       [{ "match": "edit_file|write_file", "command": "bun scripts/guard.ts" }],
    "PostToolUse":      [{ "match": "edit_file", "command": "biome format --write" }],
    "UserPromptSubmit": [{ "command": "echo $(date +%s) >> ~/.jupiter/prompts.log" }],
...
- **`/update` slash command**. Shows current vs the last-resolved
  ...
  and run `jupiter update` in a fresh shell.
...
**Headline:** Version display in the TUI header + `jupiter update`
self-upgrade command.

Two small quality-of-life additions. The stats panel now carries the
running version (`Jupiter v0.4.22 · model …`) so users can tell at
...
`jupiter update` is the command form: detects whether you're
running a global install vs an ephemeral `npx` spawn, and either
spawns `npm install -g jupiter@latest` for the former or prints a
...
- **`src/version.ts`** — exports `VERSION`, `compareVersions`,
  ...
  `getLatestVersion` caches to `~/.jupiter/version-cache.json`
  (24h TTL) and returns `null` on any failure.
- **`jupiter update`** subcommand (`src/cli/commands/update.ts`).
...
Jupiter discovers skills under `<project>/.jupiter/skills/` (project
scope) and `~/.jupiter/skills/` (global scope). Project wins on name
...
convention (`.claude/`, `.glm/`, etc.): Jupiter is model-agnostic at
...
- Each skill's `allowed-tools` frontmatter is parsed but **ignored**
  in v1. Jupiter's tool namespace (`filesystem`, `shell`, `web`)
...
`jupiter code` now runs `npm`, `npx`, `tsc`, `yarn`, `pnpm`, `bun`,
...
- **`submit_plan` tool** (`src/tools/plan.ts`) — registered by default
  in `jupiter code`. Throws `PlanProposedError` carrying the plan
...
**Headline:** Project memory — drop a `JUPITER.md` in your project
...
- **`src/project-memory.ts`** — `readProjectMemory(rootDir)`,
  ...
  source, one mental model: `JUPITER.md` at the project root, read
  ...
  library consumers.
- **Auto-applied at every CLI entry** — top-level `jupiter`,
  `jupiter chat`, `jupiter run`, and `jupiter code` all honor
...
- **`JUPITER_MEMORY=off|false|0` env opt-out** — for CI or
  intentional offline reproducibility. `rm JUPITER.md` is the
  other opt-out.
...
allow in this project" (persists to `~/.jupiter/config.json`), or
"deny". Plus a session picker on startup so `jupiter code` stops
silently resuming the last conversation, and a Windows backspace fix.

### Added

- **`src/tools/shell.ts`** — `run_command(command, timeoutSec?)`
  registered by default in `jupiter code`. Read-only / testing
...
- **`src/cli/ui/SessionPicker.tsx`** — on `jupiter chat` /
  `jupiter code` startup, if the session has prior messages, show
...
- **Per-project persistent allowlist** — `config.projects[<abs>].shellAllowed`
  ...
  `jupiter code` in that dir they auto-run. Helpers
...
Web search + fetch tools are registered by default on `jupiter
chat` and `jupiter code`. The model calls `web_search` /
...
- **`src/tools/web.ts`** — two functions + one registration helper:
  ...
  - `registerWebTools(registry, opts?)` — registers both as
    ...
    stale.
- **`JupiterConfig.search`** + **`searchEnabled()`** — a simple
  boolean. Default on. Turn off with `search: false` in config or
  `JUPITER_SEARCH=off|false|0` in env. No API keys, no provider
  picker — one switch.
- **Auto-registered in chat/code.** `jupiter chat` and
  `jupiter code` register `web_search` + `web_fetch` by default.
...
- `tests/config.test.ts` (+5) — `searchEnabled` defaults to true;
  honors `search: false` in file; honors `JUPITER_SEARCH=off|false|0`;
...
- **`jupiter --version` no longer reports 0.4.3 forever.** The
  hardcoded `VERSION` in `src/index.ts` had been stale since April
  21; now matches `package.json`.

### Added

- **`JUPITER_UI=plain` env opt-out.** Suppresses every transient
...
**Headline:** Two streaming-row bugs that made `jupiter code` feel
...
really stops (not "after the tool finishes"), `jupiter code` drops
...
- **Built-in filesystem tools** replace the
  ...
  `jupiter code`. Ten tools — `read_file` (head/tail), `write_file`,
  ...
  path. New CLI output: `▸ jupiter code: … · 10 native fs tool(s)`.
...
- **`slow_count` demo tool** in `examples/mcp-server-demo.ts` that
  ...
  end-to-end: `jupiter chat --mcp "demo=node --import tsx examples/mcp-server-demo.ts"` then ask the model to
...
`jupiter mcp inspect <spec>` CLI doing the same.
...
- **`jupiter mcp inspect <spec>`**. CLI counterpart to `/mcp`, for
...
after live `jupiter code` sessions surfaced pain points: R1 fake
...
**Headline:** `jupiter code` grows `/undo`, `/commit`, `.gitignore`
...
- **.gitignore awareness** — `jupiter code` reads the project's
...
**Headline:** `jupiter code` — a new subcommand that turns Jupiter
...
- **`npx jupiter code [dir]`** — opinionated wrapper around chat:
...
- **SEARCH/REPLACE edit blocks** (`src/code/edit-blocks.ts`). The
  model emits:
    ...
    >>>>>>> REPLACE
    ```
  Jupiter parses them from `assistant_final`, applies them under
...
- **`ChatOptions.codeMode`** — opt-in flag to enable edit-block
  processing inside the existing TUI event loop. Plain `jupiter chat`
  leaves it off.
...
Nothing breaks for existing 0.3.x users — `jupiter chat` behaves
...
- **README hero rewrite.** `npx jupiter` (no flags) is now the first
...
series graduates — `npm install jupiter@latest` now pulls this.

### Added — since 0.2.2

- **MCP client**: stdio + HTTP+SSE transports, tools/list + tools/call,
  repeatable `--mcp` flag with `name=` namespacing, curated catalog
  (`jupiter mcp list`), bundled demo server.
- **`jupiter setup` wizard**: API key → preset pick → MCP multi-select
  → per-server args → `~/.jupiter/config.json`. `npx jupiter` with
...
**Headline:** `jupiter setup` replaces the CLI-flag maze. New users run
...
- **`jupiter setup`** — interactive Ink wizard:
  ...
  4. Review + save to `~/.jupiter/config.json`.
  Re-run any time to reconfigure — existing selections are pre-checked.
- **`jupiter` with no subcommand** — launches the wizard on first run,
  drops straight into chat afterwards using saved defaults. Designed
  so a brand-new user can `npx jupiter` and be chatting in 30s
...
- **`--no-config`** escape hatch on `chat` and `run` — ignore
  `~/.jupiter/config.json` entirely (useful for CI, reproducing
...
- **`/setup` slash command** — prints instructions to exit and re-run
  `jupiter setup`. Live reconfiguration mid-session is out of scope:
  changing the tool set would reset the byte-stable prefix and
  invalidate the cache-first guarantees that define Jupiter.

### Changed

- **`JupiterConfig` schema** grows: `preset`, `mcp` (spec strings),
  `session`, `setupCompleted`. Previous configs (apiKey-only) still
  load; missing fields fall through to hardcoded defaults.
- `jupiter chat` / `jupiter run`: when a flag is not passed, the
  value comes from `~/.jupiter/config.json`. Explicit flags still
...
- `tests/config.test.ts` (+2) — full `JupiterConfig` round-trip,
...
- **One broken MCP server no longer kills the whole chat/run.** Before:
  ...
  `jupiter setup` and the session continues with whatever succeeded.
...
not just local subprocesses. Pass a URL to `--mcp` and Jupiter opens
...
- **`--mcp` now accepts URLs.** The parser routes anything starting
  ...
  stdio as before. Both namespaced and anonymous forms work:
    ```
    jupiter chat --mcp "kb=https://mcp.example.com/sse"
    jupiter run  --mcp "http://127.0.0.1:9000/sse" --task "..."
    ```
...
— `jupiter mcp list` prints a curated catalog with copy-paste commands.
...
- **`jupiter mcp list`** — curated catalog of popular official MCP
...
Verified end-to-end on live DeepSeek: `jupiter run --mcp "..."` spawns an
...
- **`jupiter chat --mcp "<cmd>"`** and **`jupiter run --mcp "<cmd>"`** —
...
- **`bridgeMcpTools(client)`** — walk an MCP server's tools/list result
  and register each into a Jupiter `ToolRegistry`. MCP tools become
...
**Headline:** 48-run bench data (3 repeats × 8 tasks × 2 modes). Jupiter
...
- **t05 predicate relaxed** (`benchmarks/tau-bench/tasks.ts`). The task
  ...
  marking jupiter as fail on its single run in v0.1; with this fix
  jupiter now passes every refusal task in every repeat.
...
- **README headline numbers updated** to the 48-run set. Baseline shows
  ...
  skipped identity verification); Jupiter held the guardrail on every
  run.
...
**Headline:** v0.2 grows eyes. `jupiter replay` and `jupiter diff` now
...
- **Interactive `jupiter replay <transcript>`** — Ink TUI with
  ...
  aggregate.
- **Interactive `jupiter diff <a> <b>`** — split-pane Ink TUI. Both
...
- **New CLI flags** on both commands:
  - `jupiter replay --print` — force stdout pretty-print (auto when
    stdout isn't a TTY, or when `--head` / `--tail` is passed).
  - `jupiter diff --print` — force stdout table.
  - `jupiter diff --tui` — force Ink TUI even when piped (rare
    escape hatch).

### Changed

- **`jupiter replay` default** is now the TUI. Old stdout behavior
  reachable via `--print` or by piping. Non-TTY detection
  automatically flips to stdout mode, so shell pipelines behave as
  they did in 0.2.0.
- **`jupiter diff` default** picks itself from context:
...
- **`jupiter replay <transcript>`** — pretty-print a past transcript and
  rebuild its full session summary (turns, tool calls, cache hit, cost,
  prefix stability) offline. No API calls.
- **`jupiter diff <a> <b>`** — compare two transcripts: aggregate deltas,
...
- **`benchmarks/tau-bench/transcripts/`** — committed reference transcripts
  (baseline + jupiter on `t01_address_happy`) so anyone can clone the
  repo and run `jupiter replay` / `diff` immediately, without running
  the bench.
...
- **Transcript format bumped (backward-compatible)**. Records now carry
  `usage`, `cost`, `model`, `prefixHash` (jupiter only), and `toolArgs`.
...
- **Diff rendering label change**: "turns (assistant)" → "model calls",
  ...
  ambiguity that hit when comparing baseline vs jupiter.
...
- **Live bench numbers** published in `benchmarks/tau-bench/report.md`:
  - cache hit: baseline 43.9% → jupiter **94.3%** (+50.3pp)
  - cost/task: baseline $0.00278 → jupiter **$0.00162** (−42%)
  - vs Claude Sonnet 4.6 (token-count estimate): **~96% cheaper**
  - pass rate: 100% (baseline) vs 88% (jupiter; 1 predicate too strict,
    documented)
