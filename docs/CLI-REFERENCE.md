...
Run `jupiter --help` (or any subcommand with `--help`) for the full flag list. Headline subcommands:

| Subcommand | What it does |
|---|---|
| `jupiter code [dir]` | Code-mode TUI — file edits, plan mode, edit-gate, project-scoped sessions |
| `jupiter chat` | Chat-only TUI — no filesystem access, no code mode |
| `jupiter run <task>` | Headless run — read prompt, execute, exit (CI-friendly) |
| `jupiter setup` | Interactive first-run config (API key, language, theme) |
| `jupiter sessions [name]` | List or open a saved session |
| `jupiter prune-sessions` | Drop sessions older than `--days N` |
| `jupiter replay <transcript>` | Re-render a JSONL transcript without calling the model |
| `jupiter diff <a> <b>` | Compare two transcripts (cost / cache / tokens) |
| `jupiter events <name>` | Tail the event log for a session |
| `jupiter stats [transcript]` | One-shot cost / cache breakdown |
| `jupiter doctor` | Health check — API reach, config, hooks, project |
| `jupiter commit` | `git add -A && git commit` with an LLM-written message |
| `jupiter mcp <list\|search\|install\|inspect\|browse>` | MCP server management |
| `jupiter index` | Build the local semantic index (Ollama or OpenAI-compatible embeddings) |
| `jupiter version` / `jupiter update` | Version info + upgrade hint |
...
| `--no-config` | Ignore `~/.jupiter/config.json` for this run |
...
| `/memory [list\|show\|forget\|clear]` | Manage pinned memory (JUPITER.md + `~/.jupiter/memory`) |
...
| Command | What it does |
|---|---|
| `/init [force]` | Scan project, synthesize a baseline `JUPITER.md` |
...
Jupiter sets DECSET 1007 (alternate-scroll) only — wheel events translate to ↑/↓ keypresses for the app, but native click/drag selection is left untouched. Pass `--no-mouse` to opt out entirely.
