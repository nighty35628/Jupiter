...
Jupiter can attach QQ to an existing `chat` or `code` session as a remote channel. QQ is not a third runtime mode.
...
- a recent Jupiter release that already includes QQ support
...
1. Jupiter asks for the QQ `App ID` in the current TUI.
...
1. Start `jupiter code` or `jupiter chat`.
2. Connect QQ once.
3. Send a message from QQ.
4. Let the local Jupiter session keep running.
...
Check that the local Jupiter session is still running and the channel is still connected:
