...
function buildPlatformNotes(version) { return {
  mac: {
    ...
    steps: [
      { cmd: 'xattr -dr com.apple.quarantine /Applications/Jupiter.app', note: { zh: '终端一行解除隔离属性', en: 'One-liner to clear the quarantine attribute' } },
  ...
  win: {
    ...
    steps: [
      ...
      { cmd: 'Get-AuthenticodeSignature .\\Jupiter_setup.exe', note: { zh: '可在 PowerShell 中校验文件 hash', en: 'Verify the file hash in PowerShell if you want' } },
    ],
  },
  linux: {
    ...
    steps: [
      { cmd: `chmod +x Jupiter_${version}_amd64.AppImage`, note: { zh: '赋予可执行权限', en: 'Mark it executable' } },
...
function DownloadHero() {
  ...
  const heroBadge =
    rxStatus === "ok" && rxVersion
      ? `Jupiter Desktop · ${rxVersion}`
      : t({ zh: "Jupiter Desktop · 正在获取版本…", en: "Jupiter Desktop · fetching version…" }, lang);
  return (
    <section className="dl-hero">
      ...
      <div className="dl-hero-grid">
        <div>
          ...
          <p className="lede">
            {t({
              zh: <>原生 <b>Tauri</b> 客户端 · 自带 Node runtime · 共享 <b>~/.jupiter</b> 配置与会话。多 tab 并行，右侧栏列出当前会话读过和改过的文件，底部 cost / cache / token 实时表盘。</>,
              en: <>Native <b>Tauri</b> client · bundled Node runtime · shares <b>~/.jupiter</b> config + history with the CLI. Multi-tab sessions, side panel listing files read / edited this session, live cost / cache / token meters along the bottom.</>,
            }, lang)}
          </p>
    ...
    <section className="section" id="cli-alt">
      ...
      <div className="section-head">
        ...
        <p className="section-sub">
          {t({
            zh: '桌面端只是 CLI 的可视化伴侣。如果你日常就在终端里，直接 npx 拉起 jupiter code 即可，缓存策略、工具协议、记忆路径完全一致。',
      ...
      <div className="copy-block" style={{maxWidth: 640}}>
        <span className="cmd"><span className="tok-cmt">$ </span>cd /path/to/my-project &amp;&amp; npx jupiter code</span>
      </div>
