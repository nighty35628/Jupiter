...
window.JUPITER_VERSION = window.JUPITER_VERSION || "";
window.JUPITER_VERSION_STATUS = window.JUPITER_VERSION
  ? "ok"
  : "loading";
...
(function fetchVersion() {
  ...
  fetch(url, { signal: ctrl.signal, cache: "no-cache" })
    ...
    .then(function (j) {
      clearTimeout(timer);
      if (j && j.version) {
        window.JUPITER_VERSION = String(j.version).replace(/^v/, "");
        window.JUPITER_VERSION_STATUS = "ok";
      } else {
        window.JUPITER_VERSION_STATUS = "failed";
      }
      window.dispatchEvent(new Event("jupiter:version"));
    })
    .catch(function () {
      clearTimeout(timer);
      window.JUPITER_VERSION_STATUS = "failed";
...
function useVersion() {
  const [v, setV] = React.useState({
    version: window.JUPITER_VERSION,
    status: window.JUPITER_VERSION_STATUS,
  });
  React.useEffect(() => {
    const handler = () =>
      setV({
    ...
    window.addEventListener("jupiter:version", handler);
    return () => window.removeEventListener("jupiter:version", handler);
  }, []);
  return v;
}
...
function detectInitialLang() {
  try {
    ...
    const stored = localStorage.getItem("jupiter.lang");
...
function LangProvider({ children }) {
  ...
  const setLang = React.useCallback((v) => {
    setLangState(v);
    try {
      localStorage.setItem("jupiter.lang", v);
