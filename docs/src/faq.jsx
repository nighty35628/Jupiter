// FAQ — based on actual README "what Jupiter deliberately doesn't do" section

const FAQS = [
  {
    q: { zh: '为什么只支持 DeepSeek？能不能换 Claude / GPT？', en: 'Why DeepSeek only? Can I swap to Claude / GPT?' },
    a: {
      zh: '这不是限制，是设计。DeepSeek 的 prefix-cache 从 prompt 第 0 字节开始指纹化，Jupiter 的循环是围绕这个不变量构建的 —— 长会话能保持 ~94% 缓存命中。挂到 Anthropic 兼容端点能拿到便宜 token，但 cache_control 标记会失效；通用 backend (Aider / Cline / Continue) 的压缩模式则会破坏字节稳定性。Coupling to one backend is the feature。',
      en: 'It\'s a design choice, not a limitation. DeepSeek\'s prefix cache fingerprints prompts from byte 0; the Jupiter loop is engineered around that invariant — long sessions hold ~94% cache hit. Pointing at an Anthropic-compatible endpoint gets you cheap tokens but breaks the cache_control markers. Generic backends (Aider / Cline / Continue) compress history, which destroys byte stability. Coupling to one backend is the feature.',
    },
  },
  {
  ...
  },
  {
    ...
    a: {
      zh: '完全同一份循环 / 协议 / ~/.jupiter 配置。桌面端 (Tauri) 自带 Node runtime，无需独立 npm install；多 tab 会话、右侧栏列出当前会话读过和改过的文件，底部显示 cost / cache / token 实时表盘。',
      en: 'Same loop, same protocol, same `~/.jupiter` config. The desktop (Tauri) bundles its own Node runtime — no separate npm install. Multi-tab sessions, side panel listing files this session read or wrote, live cost / cache / token meters along the bottom.',
    },
  },
  {
    q: { zh: '怎么开发自己的 Skill？', en: 'How do I write my own skill?' },
    a: {
      zh: '没有远程注册表，直接写文件。在 TUI 内 /skill new my-skill 生成项目级模板，--global 写到 ~/.jupiter/skills 跨项目复用。Skill 是带 frontmatter (description, runAs, allowed-tools) 的 Markdown，runAs: subagent 会在隔离子循环里运行。',
      en: 'No remote registry — just write a file. Inside the TUI run `/skill new my-skill` to scaffold a project-local skill; add `--global` to put it under `~/.jupiter/skills` for reuse across projects. Skills are Markdown with frontmatter (description, runAs, allowed-tools); `runAs: subagent` runs the body in an isolated sub-loop.',
    },
  },
  {
  ...
  },
  {
    ...
    a: {
      zh: '不能在 session 中途切。memory 路径会与陈旧的根目录纠缠。退出后 jupiter code --dir <path> 重新启动即可。',
      en: 'No — memory paths would tangle with the stale root. Exit and relaunch with `jupiter code --dir <path>`.',
    },
  },
];
