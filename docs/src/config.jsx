...
// Tabbed interface with code panels showing real Jupiter config
...
const CONFIG_TABS = [
  {
    ...
    desc: {
      zh: 'MCP 是 Jupiter 接入外部能力的一等公民通道，支持 stdio / SSE / Streamable HTTP 三种传输。每个 server 的工具会以前缀合并进统一的工具 registry，对模型透明。',
      en: 'MCP is the first-class channel for plugging external capabilities into Jupiter — supports stdio, SSE, and Streamable HTTP transports. Each server\'s tools merge into the unified registry under a prefix, transparent to the model.',
    },
    bullets: [
  ...
  },
  {
    ...
    bullets: [
      { zh: '项目级: <project>/.jupiter/skills/<name>.md', en: 'Project: <project>/.jupiter/skills/<name>.md' },
      { zh: '全局: ~/.jupiter/skills/<name>.md', en: 'Global: ~/.jupiter/skills/<name>.md' },
    ...
    files: [
      {
        name: '.jupiter/skills/review-pr.md',
        lang: 'md',
        code: `---
...
› please review the current branch`,
  ...
  },
  {
    ...
    desc: {
      zh: 'Jupiter 把"应当记住"的内容拆成两层：仓库级的 jupiter.md（提交进 git，团队共享）与用户级的 ~/.jupiter/memory.md（个人偏好，不入库）。每次会话启动时自动注入到 prompt 头部。',
      en: 'Jupiter splits "what to remember" into two layers: repo-level `jupiter.md` (checked into git, shared with the team) and user-level `~/.jupiter/memory.md` (personal preferences, kept private). Both are injected at the top of the prompt on every session.',
    },
    bullets: [
      { zh: '<project>/jupiter.md · 项目约定 · git-tracked', en: '<project>/jupiter.md · project conventions · git-tracked' },
      { zh: '~/.jupiter/memory.md · 用户偏好 · 私有', en: '~/.jupiter/memory.md · user preferences · private' },
    ...
    files: [
      {
        name: '<project>/jupiter.md',
        lang: 'md',
        code: `# jupiter.md
# Jupiter loads this on every session start.
...
- Small commits · one thing at a time`,
  ...
  },
  {
    ...
    desc: {
      zh: '一份 JSON 配置承载所有可调项。全局放 ~/.jupiter/config.json，每个项目可以再用 <project>/.jupiter/config.json 局部覆盖。',
      en: 'A single JSON file holds every knob. The global one lives at `~/.jupiter/config.json`; any project can override it locally with `<project>/.jupiter/config.json`.',
    },
    bullets: [
  ...
  "telemetry": false
      ...
      {
        name: '<project>/.jupiter/config.json',
        lang: 'json',
        code: `{
  ...
  return (
    <section className="section" id="config">
      <SecHead
        ...
        sub={t({
          zh: 'Jupiter 把可扩展性收敛到几个明确的目录与文件 —— 没有花哨的注册表，所有内容都是可读、可 diff、可入库的纯文本。',
          en: 'Jupiter collapses extensibility into a handful of well-defined directories and files. No registries, no magic — everything is readable, diffable, git-trackable plain text.',
        }, lang)}
      />
