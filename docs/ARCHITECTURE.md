...
Jupiter is **opinionated, not general**. Every abstraction is justified by a
...
|---|---|---|
| `JUPITER_PARALLEL_MAX` | `3` (hard cap `16`) | Max chunk size. |
| `JUPITER_TOOL_DISPATCH=serial` | unset | Forces serial dispatch — escape hatch. |
...
set in `.jupiter/settings.json` under the `model` key. No one-shot
...
├── code/prompt.ts          # jupiter code main system prompt
...
├── project-memory.ts       # JUPITER.md loader
├── user-memory.ts          # ~/.jupiter/memory/ store (project + global)
...
├── usage.ts                # ~/.jupiter/usage.jsonl roll-up
...
- **v0.4.x** — `jupiter code` with SEARCH/REPLACE edits, review/auto
